...
## MoEs
After opening the paper I encountered the concept of MoEs. To get myself more familiar, I read [this blog on Hugging Face](https://huggingface.co/blog/moe) (Sanseviero et al., 2023), which was really helpful—highly recommended. MoE stands for **M**ixture **o**f **E**xperts; well-known examples are DeepSeek's V2 and V3 models. It has many advantages, as the authors wrote: easy to scale to a large number of parameters, manageable costs, etc.
...
- $G\colon\R\to\R_{>0}$ a positive gating function (e.g. $\exp$ or $\text{sigmoid}$; softmax is $\exp$ followed by normalization across the $N$ experts).
...
So here $G$ could be any positive function of the score. Common choices are $\exp$, sigmoid and softmax (to be honest I had to look the last two up). Strictly, softmax is not a function $\R \to \R_{>0}$ of one score: it exponentiates and then normalizes across all $N$ experts. In this paper they use softmax and sigmoid.

Each token is then processed only by the $K$ experts whose $g_{i,t}$ is nonzero.
...
- **Normalized load**   
  $f_i$:= the share of tokens routed to expert $i$, scaled so that $f_i = 1$ for every expert when the load is perfectly balanced:
  $$
...
with small weights $\lambda_P$ and $\lambda_f$, tuned as hyperparameters.
...
> $$
> Details: write $\text{CV}^2 = \nu/\mu^2 - 1$, and use $\partial\nu/\partial z_k = 2z_k/N$ and $\partial\mu/\partial z_k = 1/N$:
> $$
> \frac{\partial}{\partial z_k}\Bigl(\frac{\nu}{\mu^2} - 1\Bigr)
> = \frac{1}{\mu^2}\cdot\frac{2z_k}{N} - \frac{2\nu}{\mu^3}\cdot\frac{1}{N}
> = \frac{2}{N}\Bigl(\frac{z_k}{\mu^2} - \frac{\nu}{\mu^3}\Bigr).
> $$
>
> On its own this gradient changes sign at $z_k = \nu/\mu$, which is above the mean ($\nu/\mu = \mu + \text{Var}/\mu$), so read naively it would push a slightly overloaded expert *up*. What rescues the intuition is that the loads have a fixed total: $\sum_i f_i = N$ always, and $\sum_i P_i = 1$ under softmax gating. Only the part of the gradient that sums to zero across experts can act, and subtracting its mean leaves
> $$
> \frac{2}{N\mu^2}\,(z_k - \mu),
> $$
> so descent pushes down every $z_k > \mu$ (overloaded expert) and pushes up every $z_k < \mu$ (underloaded expert).


**Total training objective**  
Combine with the primary task loss $\mathcal{L}_{\text{task}}$:
$$\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{task}} + \mathcal{L}_{\mathrm{balance}},$$
optionally adding $\mathcal{L}_P + \mathcal{L}_f$.
...
- Minimizing $\text{CV}^2$ drives the variance of $\{P_i\}$ or $\{f_i\}$ toward zero relative to their mean (see the derivation of $\partial \text{CV}^2/\partial z_k$ above).
- Any expert $i$ with above-average usage raises its own $P_i$ and $f_i$, increasing the penalty.
...
I found the evidence for this thinner than the claim: the gain in validation perplexity is small, and I would want to see it hold across more model scales before reading much into it. The load-balance result is the paper's main point, and that part is convincing.
...
- The balancing gradient for an expert involves all experts' totals. So updating the logit for one expert now depends on every other expert’s load. It can drown out more specialized signals.
...
Continued in [part 2](post.html?id=012-MoE2).
