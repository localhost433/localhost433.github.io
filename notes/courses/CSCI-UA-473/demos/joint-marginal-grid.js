/* AUTO-GENERATED from joint-marginal-grid.jsx by `npm run build:artifacts`. Do not edit. */
...
const JOINT = [[0.08, 0.12, 0.04, 0.06], [0.04, 0.20, 0.12, 0.04], [0.02, 0.08, 0.16, 0.04]];
...
export default function JointMarginalGrid() {
  ...
  const cells = JOINT.map((row, i) => [...row.map(v => ({
    t: f2(v),
    ...
    t: f2(rowSum(row)),
    tone: "mid",
    alpha: i === active ? 0.34 : 0.16,
    bold: i === active
  } : null]);
  cells.push([...JOINT[0].map((_, j) => colsDone ? {
    t: f2(colSum(j)),
    tone: "mid",
    alpha: 0.14
  } : null), colsDone ? {
  ...
  const cond = [JOINT[PICK].map(v => ({
    t: f2(v / rowSum(JOINT[PICK])),
    tone: "acc",
    alpha: 0.15
  }))];
  ...
  return /*#__PURE__*/React.createElement(Figure, {
    ...
    height: CH,
    fill: C.acc,
    fillOpacity: active < 0 ? 0 : 0.1,
    style: {
      transition: "y .5s ease, fill-opacity .4s ease"
    }
    ...
    step: step,
    at: S_COND,
    lines: ["The row was 0.40 of the table; rescaling", "is what makes it a distribution again."]
  }))));
}