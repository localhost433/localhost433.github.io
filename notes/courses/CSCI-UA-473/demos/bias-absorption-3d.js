...
function draw(s, w, C) {
  ...
  s.seg([-2.6, 0, 0], [2.6, 0, 0], {
    color: C.border,
    dash: [4, 4]
  });
  s.seg([0, -2.6, 0], [0, 2.6, 0], {
    color: C.border,
    dash: [4, 4]
  });
  s.arrow([0, 0, 0], [0, 0, 1.7], {
    color: C.border,
    width: 1
  });
  s.text("x₁", [2.75, 0, 0], {
  ...
  if (nz) {
    s.poly(clipHalfPlane(sq, f), {
      fill: C.pos,
      alpha: 0.2
    });
    s.poly(clipHalfPlane(sq, p => -f(p)), {
      fill: C.neg,
      alpha: 0.2
    });
  }
  ...
  if (nz) {
    ...
    const R = 2.3,
      corner = (a, b) => [0, 1, 2].map(k => R * (a * u[k] + b * v[k]));
    ...
    if (tr.length >= 2) s.seg(tr[0], tr[1], {
      color: C.fg,
      width: 2.5
    });
  ...
  LIFT_POINTS.forEach(p => {
    const at = [p.a, p.b, 1],
      bad = p.y * f([p.a, p.b]) <= 0;
    s.dot(at, {
      color: p.y > 0 ? C.pos : C.neg,
      r: p.y > 0 ? 5.5 : 5,
      shape: p.y > 0 ? "circle" : "square"
    });
    if (bad) s.ring(at, {
      color: C.fg
    });
...
export default function BiasAbsorption() {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("h2", {
  ...
  }, KNOBS.map(([id, label], i) => /*#__PURE__*/React.createElement("div", {
    key: id,
    style: {
      display: "grid",
    ...
    htmlFor: "bias-absorption-" + id
  }, label), /*#__PURE__*/React.createElement(Slider, {
    id: "bias-absorption-" + id,
    min: -3,
    max: 3,
    step: 0.1,
    value: w[i],
  ...
  }, w[i].toFixed(1))))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: "6px"
    ...
    style: buttonStyle(C)
  }, "View from above"))), /*#__PURE__*/React.createElement(Canvas3D, {
    height: H,
    interactive: true,
    view: view,
    onView: setView,
