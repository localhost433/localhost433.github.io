/* AUTO-GENERATED from complexity-tradeoff.jsx by `npm run build:artifacts`. Do not edit. */
...
const C_MIN = 1,
  C_MAX = 8,
  C_BASE = 2; // the complexity of f; base is the ghost
...
class TradeoffPlot extends Plot {
  ...
  ghost(c) {
    const g = this.g;
    g.globalAlpha = 0.28;
    this.curve(M => ein(M, c), {
      color: this.C.pos,
      width: 1.5,
      dash: [3, 4]
    });
    this.curve(M => eout(M, c), {
      color: this.C.neg,
      width: 1.5,
      dash: [3, 4]
    });
  ...
  render({
    ...
    this.label("d*", this.x(star) + 6, this.top + 12, {
      align: "left",
      color: C.muted
    });
    this.label("Error", this.left - 8, this.top + 6, {
      align: "right"
    });
    this.label("Complexity of H  →", this.x(MMAX), this.bottom + 22, {
      align: "right"
    });
    return this;
  }
...
const VERDICT = (c, star, base) => {
  ...
  return `The falling curve has moved${star > base ? " up" : " down"}; the rising one has not moved at all. ` + `The optimum has gone ${dir}, from d* ≈ ${base.toFixed(1)} to ${star.toFixed(1)}.`;
};
export default function App() {
  ...
  const star = bestM(c),
    base = bestM(C_BASE);
  React.useEffect(() => {
    if (ref.current) new TradeoffPlot(ref.current, C).render({
      c
    });
  });
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sr-only"
  }, "The error-against-complexity tradeoff, with a control for the complexity of the target function: raising it lifts the in-sample curve, leaves the model-complexity penalty unchanged, and slides the optimum to the right."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px",
    ...
    style: labelStyle(C)
  }, "complexity of f"), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: C_MIN,
    max: C_MAX,
    step: "0.1",
  ...
  }, c === C_BASE ? "baseline" : c < C_BASE ? "simpler f" : "more complex f"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    style: buttonStyle(C, c === C_BASE),
    onClick: () => setC(C_BASE)
  }, "reset"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    style: buttonStyle(C, c === C_MAX),
    onClick: () => setC(C_MAX)
  }, "a much harder f")), /*#__PURE__*/React.createElement("canvas", {
    ref: ref,
    style: {
      width: "100%",
      maxWidth: W,
  ...
  }, "d* = ", star.toFixed(1), "    best achievable E_out = ", eout(star, c).toFixed(3)), /*#__PURE__*/React.createElement("p", {
    style: {
      ...labelStyle(C),
      margin: 0,
      lineHeight: 1.5
    }
  }, VERDICT(c, star, base)), /*#__PURE__*/React.createElement("p", {
    style: {
      ...labelStyle(C),
      margin: 0,
      lineHeight: 1.5
    }
  }, c !== C_BASE && "Faint dashed curves are where things sat at the baseline. ", "A more complex target is harder to fit, so in-sample error stays higher at every complexity of H - that is question 2 getting harder. The model-complexity penalty does not budge, because it comes from the bound and the bound depends on |H| and N, never on f. Their sum therefore bottoms out further right: ", /*#__PURE__*/React.createElement("strong", null, "you have to spend more capacity"), ". It also bottoms out higher, which is the part that is easy to miss \u2014 the extra capacity does not buy back what the harder target cost you, it only stops you paying twice. No axis has numbers on it: this is the shape of the relationship, and the measured version is the figure below."));
}