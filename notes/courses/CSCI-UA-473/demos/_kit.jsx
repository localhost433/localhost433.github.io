...
const LIGHT = { pos: "#1D9E75", neg: "#D85A30", acc: "#7F77DD" };
const DARK  = { pos: "#34D399", neg: "#FB923C", acc: "#A78BFA" };
...
function renderInline(text, C) {
  ...
  return String(text).split(/(\*\*[^*]+\*\*|`[^`]+`)/g).map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return <code key={i} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: "3px", padding: "1px 3px", color: C.fg }}>{part.slice(1, -1)}</code>;
    }
...
export function Mcq({ questions: rawQuestions = [] }) {
  ...
  const navButton = (disabled) => ({ ...buttonStyle(C), opacity: disabled ? 0.5 : 1, cursor: disabled ? "default" : "pointer" });
  const choiceButton = (choice, index) => {
    const picked = answered && index === pick;
    const correct = answered && choice.correct;
    return {
      ...buttonStyle(C), display: "block", width: "100%", margin: "0 0 8px", textAlign: "left",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px", color: C.fg }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px" }}>
        <button type="button" onClick={() => go(-1)} disabled={cur === 0}
          aria-label="Previous question" style={navButton(cur === 0)}>‹ Prev</button>
        <span aria-live="polite">Question {cur + 1} of {n}</span>
