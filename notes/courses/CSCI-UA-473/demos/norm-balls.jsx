...
const NAME = (p) => (Math.abs(p - 1) < 0.02 ? "L₁ — the lasso's diamond"
  : Math.abs(p - 2) < 0.02 ? "L₂ — ridge's circle"
  : p < 1 ? "p < 1 — non-convex, corners sharper still"
  : p > 2 ? "p > 2 — rounder, heading for a square at p = ∞"
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px", color: C.fg }}>
      <h2 className="sr-only">Unit balls of the Lp norms, sweeping p between one half and six, with the L1 and L2 balls held as reference.</h2>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        <span style={labelStyle(C)}>p</span>
        <input type="range" min="0.5" max="6" step="0.05" value={p} onChange={(e) => setP(+e.target.value)}
          aria-label="Norm exponent p" style={{ flex: "1 1 150px", minWidth: "130px", accentColor: C.acc }} />
        <span style={{ ...readoutStyle(C), margin: 0, minWidth: "4em" }}>p = {p.toFixed(2)}</span>
        <button type="button" style={buttonStyle(C, Math.abs(p - 1) < 0.02)} onClick={() => setP(1)}>p = 1</button>
        <button type="button" style={buttonStyle(C, Math.abs(p - 2) < 0.02)} onClick={() => setP(2)}>p = 2</button>
      </div>
      <canvas ref={ref} style={{ width: "100%", maxWidth: W, aspectRatio: "1 / 1" }}
      ...
      <p style={{ ...labelStyle(C), margin: 0, lineHeight: 1.5 }}>
        ...
        to land on an axis rather than beside it.
      </p>
    </div>
  );
}
