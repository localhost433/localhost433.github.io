...
const CW = 56, CH = 30, HX = 62, HY = 28;
const XX = 96, XY = 76;                       // top-left of the X block, headers included
const CELLS_Y = XY + HY;                      // where X's data cells actually start
const XCELLS = XX + HX;                       // and their left edge
const XW = 3 * CW;

const WX = 372, WY = CELLS_Y + (4 * CH - 3 * CH) / 2;
const YX = 500;

const STEPS = 4;
const S_W = 1, S_Y = 2, S_NOTE = 3;

export default function DesignMatrixAnatomy() {
  const C = useClassColors();
  const box = React.useRef(null);
  const step = useReveal(box, STEPS, { ms: 1000 });
  useTypeset([C, step]);

  const f1 = (v) => v.toFixed(1);
  // the leading column of ones is the only part of X that is not data
  const xCells = ROWS.map((r) => [
    { t: "1", tone: "acc", alpha: 0.22 },
    { t: f1(r[0]) },
    { t: String(r[1]) },
  ]);
  const yCells = Y.map((v) => [{ t: f1(v), tone: "pos", alpha: 0.16 }]);

  const bracket = (x, y, h, cols, cw, at) => (
    <>
      <Bracket x={x - 6} y={y} h={h} C={C} step={step} at={at} />
      <Bracket x={x + cols * cw + 6} y={y} h={h} side="right" C={C} step={step} at={at} />
    </>
  );
  const op = (t, x, y, at) => (
    <text x={x} y={y} textAnchor="middle" fontSize={20} fill={C.muted}
      style={{ transition: "opacity .45s ease", opacity: step >= at ? 1 : 0 }}>{t}</text>
  );
    ...
    >
      <div ref={box}>
        <FigureSvg viewBox="0 0 764 372" maxWidth={764}
          ...
          {/* ---- X ---- */}
          <TableGrid x={XX} y={XY} cw={CW} ch={CH} cells={xCells} C={C} size={12.5} texHead
            colHead={["1", "\\text{income}", "\\text{years}"]}
            rowHead={["x^1", "x^2", "x^3", "x^4"]} />
          {bracket(XCELLS, CELLS_Y, 4 * CH, 3, CW, 0)}
          <Lines x={XCELLS + XW / 2} y={XY - 22} size={12} fill={C.fg} anchor="middle"
            lines={["X — the design matrix"]} />
          <Lines x={XX - 44} y={CELLS_Y + 2 * CH} size={11} fill={C.muted} anchor="middle"
            lines={["one row", "= one", "example"]} />
          <TeX x={XCELLS + CW / 2} y={CELLS_Y + 4 * CH + 34} w={200} anchor="middle" size={11.5}
            fill={C.acc} tex="\text{the absorbed bias}" />

          {/* ---- w ---- */}
          {op("\u00b7", 344, CELLS_Y + 2 * CH + 6, S_W)}
          <g style={{ transition: "opacity .45s ease", opacity: step >= S_W ? 1 : 0 }}>
            {["w_0", "w_1", "w_2"].map((t, i) => (
              <g key={t}>
                <rect x={WX} y={WY + i * CH} width={CW} height={CH}
                  fill={i === 0 ? C.acc : C.bg} fillOpacity={i === 0 ? 0.22 : 1}
                  stroke={C.border} strokeWidth="1" />
                <TeX x={WX + CW / 2} y={WY + i * CH + CH / 2 + 4} w={CW * 2} anchor="middle"
                  size={13.5} fill={i === 0 ? C.acc : C.fg} tex={t} />
              </g>
            ))}
          </g>
          {bracket(WX, WY, 3 * CH, 1, CW, S_W)}
          <Lines x={WX + CW / 2} y={WY - 16} size={12} fill={C.fg} anchor="middle" step={step} at={S_W}
            lines={["w — what is learned"]} />

          {/* ---- y ---- */}
          {op("\u2248", 464, CELLS_Y + 2 * CH + 6, S_Y)}
          <TableGrid x={YX} y={CELLS_Y} cw={CW + 8} ch={CH} cells={yCells} C={C} size={12.5}
            step={step} at={S_Y} />
          {bracket(YX, CELLS_Y, 4 * CH, 1, CW + 8, S_Y)}
          <Lines x={YX + (CW + 8) / 2} y={CELLS_Y - 16} size={12} fill={C.pos} anchor="middle"
            step={step} at={S_Y} lines={["y — credit line"]} />

          {/* ---- the shapes, named ---- */}
          <g style={{ transition: "opacity .45s ease", opacity: step >= S_NOTE ? 1 : 0 }}>
            <TeX x={604} y={CELLS_Y + 8} w={170} size={13} fill={C.fg}
              tex="X:\; N \times (d{+}1)" />
            <TeX x={604} y={CELLS_Y + 42} w={170} size={13} fill={C.fg}
              tex="w:\; (d{+}1) \times 1" />
            <TeX x={604} y={CELLS_Y + 76} w={170} size={13} fill={C.fg}
              tex="y:\; N \times 1" />
            <Lines x={604} y={CELLS_Y + 106} size={11} fill={C.muted}
              lines={["here N = 4, d = 2"]} />
          </g>

          <line x1={64} y1={268} x2={700} y2={268} stroke={C.border} strokeWidth="1"
            style={{ transition: "opacity .45s ease", opacity: step >= S_NOTE ? 1 : 0 }} />

          {/* ---- the two jobs the weight vector is doing ---- */}
          <g style={{ transition: "opacity .45s ease", opacity: step >= S_NOTE ? 1 : 0 }}>
            <TeX x={64} y={300} w={330} size={13} fill={C.acc}
              tex="w_0 \;\text{— the intercept: how far the hyperplane}" />
            <Lines x={64} y={318} size={12.5} fill={C.acc} lines={["sits from the origin"]} />
            <TeX x={404} y={300} w={340} size={13} fill={C.fg}
              tex="[w_1,\dots,w_d] \;\text{— the hyperplane's}" />
            <Lines x={404} y={318} size={12.5} fill={C.fg} lines={["orientation in d dimensions"]} />
            <TeX x={64} y={350} w={660} size={13} fill={C.muted}
