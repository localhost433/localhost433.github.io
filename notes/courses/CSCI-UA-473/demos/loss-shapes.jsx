...
function regression(p, C) {
  p.grid({ yTicks: 4 });
  p.axes({ atY: 0, atX: 0 });
  p.curve((r) => r * r, { color: C.acc, width: 2.5 });
  p.curve((r) => Math.abs(r), { color: C.muted, width: 1.5, dash: [5, 4] });
...
function classification(p, C) {
  ...
  p.polyline([[-3, 1], [-0.02, 1]], { color: C.fg, width: 2.5 });
  p.polyline([[0.02, 0], [3, 0]], { color: C.fg, width: 2.5 });
  // the jump itself, so the discontinuity is visible rather than implied
  p.polyline([[0, 0], [0, 1]], { color: C.fg, width: 1.2, dash: [3, 3] });
  p.dot(0, 0, { color: C.fg, r: 3.5 });
  ...
  p.labelAt("right", 2.3, 0, { dy: 26, color: C.pos, size: 10.5 });
}
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexWrap: "wrap", gap: "18px", justifyContent: "center" }}>
        <div>
          <p style={{ margin: "0 0 4px", fontSize: "12.5px", color: C.muted }}>regression: a function of the residual</p>
          <PlotFigure width={W} height={H} draw={(p) => regression(p, C)}
        ...
        <div>
          <p style={{ margin: "0 0 4px", fontSize: "12.5px", color: C.muted }}>classification: a function of the margin</p>
          <PlotFigure width={W} height={H} draw={(p) => classification(p, C)}
            plotOpts={{ xDomain: [-3, 3], yDomain: [0, 3], pad: 30, padTop: 14, padRight: 16 }}
            labels={[{ at: [0, 0], dy: 38, anchor: "middle", tex: "y \\cdot h(x)", size: 11.5, color: C.muted }]}
