/* AUTO-GENERATED from bin-to-learning-map.jsx by `npm run build:artifacts`. Do not edit. */
...
const SAMPLE = 10;
const ROWS_MAP = [["\\text{one marble}", "\\text{one input point } x \\in \\mathcal X"], ["\\text{the whole bin}", "\\text{the whole input space } \\mathcal X"], ["\\text{the marble is red}", "h(x) \\neq f(x)"], ["\\mu\\text{, the fraction red in the bin}", "E_{\\text{out}}(h) = \\mathbb P\\big[h(x) \\neq f(x)\\big]"], ["\\text{a drawn sample of } N \\text{ marbles}", "\\text{the training set } D"], ["\\nu\\text{, the fraction red in it}", "E_{\\text{in}}(h) = \\tfrac{1}{N}\\sum_i \\big[h(x_i) \\neq f(x_i)\\big]"], ["\\mu \\text{ is what we cannot see}", "f \\text{ is what we cannot see}"]];
export default function BinToLearningMap() {
  const C = useClassColors();
  useTypeset([C]);
  const marble = (cx, cy, red, key) => /*#__PURE__*/React.createElement("circle", {
    key: key,
    cx: cx,
    cy: cy,
    r: R,
    fill: red ? C.neg : C.acc
  });
  const bin = [];
  ...
  return /*#__PURE__*/React.createElement(Figure, {
    ...
    fontSize: "12.5",
    fill: C.muted
  }, "the bin"), /*#__PURE__*/React.createElement("rect", {
    x: 16,
    y: 38,
    width: COLS * GAP + 6,
    height: ROWS * GAP + 6,
    ...
    stroke: C.fg,
    strokeWidth: "1.5"
  }), bin, /*#__PURE__*/React.createElement(Arrow, {
    from: [92, 20],
    to: [36, 46],
    C: C,
    tone: "neg",
    ...
    length: SAMPLE
  }, (_, i) => marble(30 + i * GAP, 262, SAMPLE_RED.has(i), "s" + i)), /*#__PURE__*/React.createElement(TeX, {
    x: 16,
    y: 300,
    w: 320,
    size: 13,
    ...
    fill: C.muted,
    fontStyle: "italic"
  }, "ball and bin"), /*#__PURE__*/React.createElement("text", {
    x: 560,
    y: 28,
    fontSize: "12",
    fill: C.muted,
    fontStyle: "italic"
  }, "learning"), /*#__PURE__*/React.createElement("line", {
    x1: 300,
    y1: 36,
    x2: 756,
    y2: 36,
