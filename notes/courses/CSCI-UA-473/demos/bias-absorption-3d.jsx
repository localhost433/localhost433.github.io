...
function draw(canvas, w, view, C) {
  ...
  const Wd = canvas.clientWidth || 640, dpr = window.devicePixelRatio || 1;
  canvas.width = Wd * dpr; canvas.height = H * dpr;
  const g = canvas.getContext("2d");
  g.setTransform(dpr, 0, 0, dpr, 0, 0); g.clearRect(0, 0, Wd, H);
  const cam = { cx: Wd / 2, cy: H * 0.6, s: Math.min(Wd, H) / 6.2 };
  const wv = [w[1], w[2], w[0]];

  g.font = "13px sans-serif"; g.strokeStyle = C.border; g.lineWidth = 1; g.setLineDash([4, 4]);
  seg(g, [-2.6, 0, 0], [2.6, 0, 0], cam, yaw, pitch);
  seg(g, [0, -2.6, 0], [0, 2.6, 0], cam, yaw, pitch); g.setLineDash([]);
  arrow(g, [0, 0, 0], [0, 0, 1.7], cam, yaw, pitch, C.border, 1);
  g.fillStyle = C.muted;
  let q = proj(2.75, 0, 0, cam, yaw, pitch); g.fillText("x₁", q[0], q[1]);
  q = proj(0, 2.75, 0, cam, yaw, pitch); g.fillText("x₂", q[0], q[1]);
  q = proj(0, 0, 1.8, cam, yaw, pitch); g.fillText("x₀", q[0] + 4, q[1]);
  q = proj(0, 0, 0, cam, yaw, pitch); g.fillText("origin", q[0] + 6, q[1] + 14);
  ...
  q = proj(SQ, SQ, 1, cam, yaw, pitch); g.fillStyle = C.muted;
  ...
  if (nz) {
    ...
    if (tr.length >= 2) { g.strokeStyle = C.fg; g.lineWidth = 2.5; seg(g, tr[0], tr[1], cam, yaw, pitch); }
...
function readoutStyle(C) {
  return { color: C.muted, fontSize: "13px", lineHeight: 1.6,
    fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", margin: "8px 0 0" };
}

export default function BiasAbsorption() {
  const C = useClassColors();
  const [w, setW] = React.useState([0, 0.8, -1]);
  const [view, setView] = React.useState({ yaw: -0.7, pitch: 0.42 });
  ...
  const onPointerUp = () => { drag.current = null; cv.current.style.cursor = "grab"; };
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", color: C.fg }}>
        ...
        <canvas ref={cv} role="img" aria-label="3D lifted data, shelf, and separating plane"
          onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp}
          style={{ width: "100%", height: H, border: `0.5px solid ${C.border}`, borderRadius: "6px", cursor: "grab", touchAction: "none" }} />
