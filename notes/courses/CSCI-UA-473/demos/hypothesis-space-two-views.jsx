...
const STEPS = 4;
const S_H = 1, S_F = 2, S_G = 3;

// ---- left panel: the input space ------------------------------------------
const LX = 44, LY = 76, LW = 300, LH = 216;
const px = (u) => LX + u * LW;
...
const HYPS = [
  ...
  { a: 0.50, b: -0.10 },
  { a: 0.42, b: 0.02 },
  { a: 0.38, b: 0.08 },   // the one that separates the sample: g
  { a: 0.28, b: 0.25 },
];
const G = 3;
...
const RX = 404, RY = 76, RW = 336, RH = 216;
const EC = [568, 208], ER = [140, 80];          // the ℍ ellipse
const DOTS = [[486, 216], [506, 172], [548, 200], [600, 218], [652, 238]];
const FDOT = [694, 112];

const FADE = { transition: "opacity .5s ease" };
const on = (step, at) => ({ ...FADE, opacity: step >= at ? 1 : 0 });

export default function HypothesisSpaceTwoViews() {
  const C = useClassColors();
  const box = React.useRef(null);
  const step = useReveal(box, STEPS, { ms: 1100 });
  useTypeset([C, step]);

  const line = ({ a, b }) => `${px(0)},${py(a)} ${px(1)},${py(a + b)}`;
  const fPath = Array.from({ length: 41 }, (_, i) => {
    const u = i / 40;
  ...
  return (
    ...
    >
      <div ref={box}>
        <FigureSvg viewBox="0 0 764 372" maxWidth={764}
          ...
          <Lines x={LX} y={44} size={12.5} fill={C.fg} lines={["a hypothesis is a boundary on 𝒳"]} />
          <Lines x={RX} y={44} size={12.5} fill={C.fg} lines={["a hypothesis is a point in a space of functions"]} />

          {/* ---------- left: the input space ---------- */}
          <rect x={LX} y={LY} width={LW} height={LH} fill="none" stroke={C.border} strokeWidth="1" />
          <text x={LX + LW} y={LY + LH + 16} textAnchor="end" fontSize="11" fill={C.muted}>𝒳</text>

          {/* the hypothesis set, as a fan of candidates */}
          <g style={on(step, S_H)}>
            {HYPS.map((h, i) => (
              <polyline key={i} points={line(h)} fill="none" stroke={C.muted}
                strokeWidth="1.25" strokeOpacity={i === G ? 0 : 0.7} />
            ))}
          </g>

          {/* the target, which no member of the fan can match */}
          <polyline points={fPath} fill="none" stroke={C.neg} strokeWidth="2"
            strokeDasharray="6 4" style={on(step, S_F)} />
          <text x={px(0.10)} y={py(F(0.10)) - 10} textAnchor="middle" fontSize="13" fill={C.neg}
            fontStyle="italic" style={on(step, S_F)}>f</text>

          {/* the chosen hypothesis */}
          <polyline points={line(HYPS[G])} fill="none" stroke={C.acc} strokeWidth="2.75"
            style={on(step, S_G)} />
          <text x={px(0.72)} y={py(HYPS[G].a + 0.72 * HYPS[G].b) - 11} textAnchor="middle" fontSize="13"
            fill={C.acc} fontStyle="italic" style={on(step, S_G)}>g</text>

          {/* the sample, which is on screen from the start: it is the only thing given */}
          {POS.map(([u, v], i) => <circle key={"p" + i} cx={px(u)} cy={py(v)} r="4.5" fill={C.pos} />)}
          {NEG.map(([u, v], i) => <circle key={"n" + i} cx={px(u)} cy={py(v)} r="4.5" fill={C.neg} />)}
          <text x={LX + 6} y={LY + 16} fontSize="11" fill={C.muted}>D — the sample</text>

          {/* ---------- right: the space of functions ---------- */}
          <rect x={RX} y={RY} width={RW} height={RH} rx="14" fill="none"
            stroke={C.border} strokeWidth="1" strokeDasharray="5 4" />
          <text x={RX + 10} y={RY + 18} fontSize="11" fill={C.muted}>all functions 𝒳 → 𝒴</text>

          <g style={on(step, S_H)}>
            <ellipse cx={EC[0]} cy={EC[1]} rx={ER[0]} ry={ER[1]}
              fill={C.acc} fillOpacity="0.12" stroke={C.acc} strokeWidth="1.5" strokeOpacity="0.55" />
            <TeX x={EC[0] - ER[0] + 16} y={EC[1] - ER[1] + 22} w={200} size={12.5} fill={C.acc}
              tex="\mathbb H \;-\; \text{the lines}" />
            {DOTS.map(([x, y], i) => (
              <circle key={i} cx={x} cy={y} r="4" fill={C.muted} fillOpacity={i === G ? 0 : 0.95} />
            ))}
          </g>

          <g style={on(step, S_F)}>
            <circle cx={FDOT[0]} cy={FDOT[1]} r="5" fill={C.neg} />
            <text x={FDOT[0] - 10} y={FDOT[1] - 10} textAnchor="end" fontSize="12" fill={C.neg}>f</text>
          </g>

          <g style={on(step, S_G)}>
            <line x1={DOTS[G][0]} y1={DOTS[G][1]} x2={FDOT[0]} y2={FDOT[1]}
              stroke={C.neg} strokeWidth="1.25" strokeDasharray="4 4" strokeOpacity="0.8" />
            <circle cx={DOTS[G][0]} cy={DOTS[G][1]} r="6" fill={C.acc} />
            <text x={DOTS[G][0] + 10} y={DOTS[G][1] + 16} fontSize="12" fill={C.acc}>g</text>
            <TeX x={RX + RW} y={RY + RH + 20} w={300} anchor="end" size={12} fill={C.neg}
              tex="\text{the gap no sample can close: } f \notin \mathbb H" />
          </g>

          {/* ---------- the correspondence ---------- */}
          <g style={on(step, S_G)}>
            <path d={`M ${px(0.99)} ${py(HYPS[G].a + HYPS[G].b)} C 380 ${py(HYPS[G].a + HYPS[G].b)}, 380 ${DOTS[G][1]}, ${DOTS[G][0] - 10} ${DOTS[G][1]}`}
              fill="none" stroke={C.acc} strokeWidth="1" strokeDasharray="3 3" strokeOpacity="0.7" />
          </g>

          <TeX x={382} y={348} w={720} anchor="middle" size={13.5} fill={C.fg} step={step} at={S_G}
