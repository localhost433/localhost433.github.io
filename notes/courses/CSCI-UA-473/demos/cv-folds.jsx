...
const K = 5, X0 = 112, CW = 60, GAP = 4, Y0 = 52, RH = 32, CH = 24;
const cellX = (k) => X0 + k * (CW + GAP);
const BAR_END = cellX(K - 1) + CW;

function Bar({ y, val, C, label }) {
  return (
    <>
      {Array.from({ length: K }, (_, k) => (
        <Box key={k} x={cellX(k)} y={y} w={CW} h={CH} C={C} tone={k === val ? "acc" : "pos"} rx={3}
          lines={[{ t: k === val ? "val" : "train", size: 11, fill: k === val ? C.acc : C.muted, weight: k === val ? 600 : 400 }]} />
      ))}
      <Sym x={20} y={y + 16} text={label} size={12} fill={C.muted} />
    </>
  );
}
...
export default function CvFolds() {
  ...
  const FY = 262, LOO = 20, LY = 390, LW = (BAR_END - X0 - (LOO - 1) * 2) / LOO;
  ...
  return (
    ...
    >
      <FigureSvg viewBox="0 0 760 430" maxWidth={740}
        ...
        {Array.from({ length: K }, (_, k) => (
          <Sym key={k} x={cellX(k) + CW / 2} y={40} text={`D_${k + 1}`} size={12} fill={C.muted} anchor="middle" />
        ))}
        {Array.from({ length: K }, (_, j) => (
          <g key={j}>
            <Bar y={rowY(j)} val={j} C={C} label={`phase ${j + 1}`} />
            <Arrow from={[BAR_END + 8, rowY(j) + CH / 2]} to={[BAR_END + 44, rowY(j) + CH / 2]} C={C} width={1.2} />
            <Sym x={BAR_END + 52} y={rowY(j) + 17} text={`E_{val}^{${j + 1}}`} size={13} fill={C.fg} />
          </g>
        ))}

        <path d={`M${BAR_END + 104},${Y0 + 2} L${BAR_END + 112},${Y0 + 2} L${BAR_END + 112},${rowY(K - 1) + CH - 2} L${BAR_END + 104},${rowY(K - 1) + CH - 2}`}
          fill="none" stroke={C.muted} strokeWidth="1.3" />
        <Arrow from={[BAR_END + 112, mid]} to={[566, mid]} C={C} width={1.3} />
        <Box x={568} y={mid - 30} w={184} h={60} C={C} tone="acc" />
        <TeX x={660} y={mid + 6} w={184} anchor="middle" size={13.5} fill={C.fg}
          tex={"E_{\\text{cv}}(\\lambda) = \\frac{1}{5}\\sum_{j=1}^{5} E_{\\text{val}}^{j}"} />

        <Arrow from={[660, mid + 30]} to={[660, FY - 12]} C={C} width={1.3} />
        <Box x={580} y={FY - 10} w={160} h={64} C={C}
          lines={[{ t: "repeat for each", size: 12 }, { t: "candidate λ_1, …, λ_M;", size: 12 }, { t: "keep the smallest E_{cv}", size: 12 }]} />

        <Arrow from={[578, FY + 12]} to={[BAR_END + 6, FY + 12]} C={C} tone="acc" width={1.5} />
        <Sym x={(BAR_END + 578) / 2} y={FY + 4} text="λ* fixed" size={12} fill={C.acc} anchor="middle" />
        {Array.from({ length: K }, (_, k) => (
          <Box key={k} x={cellX(k)} y={FY} w={CW} h={CH} C={C} tone="pos" rx={3}
            lines={[{ t: "train", size: 11, fill: C.muted }]} />
        ))}
        <Lines x={20} y={FY + 11} lines={["fold back:", "all N points"]} size={12} fill={C.muted} lh={1.25} />
        <Arrow from={[(X0 + BAR_END) / 2, FY + CH + 2]} to={[(X0 + BAR_END) / 2, FY + CH + 30]} C={C} width={1.3} />
        <Box x={(X0 + BAR_END) / 2 - 60} y={FY + CH + 32} w={120} h={30} C={C} tone="solid"
          lines={[{ t: "final g", size: 13, weight: 600 }]} />

        <line x1={20} y1={LY - 30} x2={740} y2={LY - 30} stroke={C.border} strokeWidth="1" />
        <Lines x={20} y={LY - 8} lines={["K = N (leave-one-out): N phases, each validating a single point"]} size={12} fill={C.muted} />
        {Array.from({ length: LOO }, (_, i) => (
          <rect key={i} x={X0 + i * (LW + 2)} y={LY + 4} width={LW} height={18} rx={2}
            fill={i === 6 ? C.acc : C.pos} fillOpacity={i === 6 ? 0.55 : 0.14}
            stroke={i === 6 ? C.acc : C.pos} strokeWidth="1" />
        ))}
        <Sym x={20} y={LY + 17} text="phase j" size={12} fill={C.muted} />
        <Arrow from={[BAR_END + 8, LY + 13]} to={[BAR_END + 44, LY + 13]} C={C} width={1.2} />
        <Sym x={BAR_END + 52} y={LY + 18} text="e_j, then average the N of them" size={12.5} fill={C.fg} />
      </FigureSvg>
    </Figure>
  );
}
