...
const NAME = p => p === Infinity ? "L∞ — the square; every point of the boundary is a coordinate at its maximum" : Math.abs(p - 1) < 0.02 ? "L₁ — the lasso's diamond" : Math.abs(p - 2) < 0.02 ? "L₂ — ridge's circle" : p < 1 ? "p < 1 — non-convex, corners sharper still" : p > 2 ? "p > 2 — rounder, the corners sliding off the axes towards the square" : "between the two taught cases";
...
export default function App() {
  ...
  const preset = target => /*#__PURE__*/React.createElement("button", {
    type: "button",
    key: String(target),
    style: buttonStyle(C, p === target),
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sr-only"
  }, "Unit balls of the Lp norms, sweeping p between one half and infinity, with the L1 and L2 balls held as reference."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px",
    ...
    style: labelStyle(C)
  }, "1/p"), /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: S_MIN,
    max: S_MAX,
    step: "0.005",
  ...
  }, "p = ", SHOW(p)), [1, 2, Infinity].map(preset)), /*#__PURE__*/React.createElement("canvas", {
    ref: ref,
    style: {
      width: "100%",
      maxWidth: W,
  ...
  }, "Every point on a coloured curve is exactly one unit away from the origin under that norm; the two grey outlines are p = 1 and p = 2, held for reference. The feature worth carrying into L5 is the corner. At p = 1 the ball touches each axis at a sharp point, and a point on an axis is a coefficient that is exactly zero. At p = 2 the boundary is smooth everywhere, so there is no reason for a solution to land on an axis rather than beside it. At p = \u221E the corners are back, but at the diagonals, where both coefficients are equal and neither is zero \u2014 which is why the endpoint that produces sparsity is p = 1 and not simply \"a pointy ball\"."), /*#__PURE__*/React.createElement("p", {
    style: {
      ...labelStyle(C),
      margin: 0,
      lineHeight: 1.5
    }
  }, "\u2016w\u2016", /*#__PURE__*/React.createElement("sub", null, "\u221E"), " = max", /*#__PURE__*/React.createElement("sub", null, "i"), " |w", /*#__PURE__*/React.createElement("sub", null, "i"), "|, the largest coordinate. This is a norm on a ", /*#__PURE__*/React.createElement("em", null, "vector"), "; the largest singular value is the operator norm of a", /*#__PURE__*/React.createElement("em", null, " matrix"), ", a different object that happens to share the \"sup over something\" definition."));
}