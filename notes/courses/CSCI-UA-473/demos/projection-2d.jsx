...
const W = 380, H = 380, R = 5;

const proj = (x, a) => {
  const aa = a[0] * a[0] + a[1] * a[1];
  const k = aa === 0 ? 0 : (a[0] * x[0] + a[1] * x[1]) / aa;
  return { k, p: [k * a[0], k * a[1]] };
};

class VectorPlot extends Plot {
  constructor(canvas, colors) {
    super(canvas, { width: W, height: H, pad: 0, padTop: 0, padRight: 0,
      xDomain: [-R, R], yDomain: [-R, R], colors });
  }

  arrow(to, { color, width = 2.5, label }) {
    const g = this.g, x0 = this.x(0), y0 = this.y(0), x1 = this.x(to[0]), y1 = this.y(to[1]);
    ...
    g.lineTo(x1 - 10 * Math.cos(ang + 0.38), y1 - 10 * Math.sin(ang + 0.38));
    g.fill();
    if (label) this.label(label, x1 + 8 * Math.cos(ang), y1 + 8 * Math.sin(ang) - 4, { color, size: 13 });
    return this;
  }

  // the square tick that says "this angle is exactly 90 degrees"
  rightAngle(at, u, v, size = 12) {
    const g = this.g, px = this.x(at[0]), py = this.y(at[1]);
    const n = (d) => { const m = Math.hypot(d[0], d[1]) || 1; return [d[0] / m, -d[1] / m]; };
    const [ux, uy] = n(u), [vx, vy] = n(v);
    g.strokeStyle = this.C.muted; g.lineWidth = 1.25;
    g.beginPath();
  ...
  render({ x, a }) {
    ...
    this.polyline([[-far[0], -far[1]], far], { color: C.acc, width: 1, dash: [4, 4] });

    // the residual, which is the whole point
    this.polyline([[x[0], x[1]], p], { color: C.neg, width: 1.75, dash: [5, 4] });
    this.rightAngle(p, [a[0], a[1]], [x[0] - p[0], x[1] - p[1]]);

    this.arrow(a, { color: C.acc, width: 1.75, label: null });
    this.labelAt("a", a[0], a[1], { dx: 10, dy: 16, color: C.acc, size: 13 });
    this.arrow(x, { color: C.pos, label: "x" });
    this.arrow(p, { color: C.fg, width: 2.5, label: null });
    this.dot(p[0], p[1], { color: C.fg, r: 4 });
    this.labelAt("proj", p[0], p[1], { dx: -8, dy: -10, color: C.fg, size: 13, align: "right" });
    return this;
  }
}
...
export default function App() {
  ...
  React.useEffect(() => {
    if (ref.current) new VectorPlot(ref.current, C).render({ x, a });
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px", color: C.fg }}>
      <h2 className="sr-only">Projection of a vector x onto the line spanned by a, with the residual drawn and its orthogonality marked.</h2>
      ...
      <p style={{ ...readoutStyle(C), margin: 0, lineHeight: 1.7 }}>
        ...
        proj = [{p[0].toFixed(2)}, {p[1].toFixed(2)}]   residual = [{res[0].toFixed(2)}, {res[1].toFixed(2)}]<br />
      ...
      <p style={{ ...labelStyle(C), margin: 0, lineHeight: 1.5 }}>
        ...
        X in place of the line through a, which is why the normal equations read
        Xᵀ(y − ŷ) = 0.
      </p>
    </div>
  );
