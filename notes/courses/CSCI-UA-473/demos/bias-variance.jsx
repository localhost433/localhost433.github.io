...
const W = 420, H = 260, SW = 300, SH = 260, MAXM = 9, K = 40, N = 10, SIGMA = 0.2;
...
class FitsPlot extends Plot {
  ...
  render({ fits, gx, gbar }) {
    ...
    g.globalAlpha = 0.16;
    fits.forEach((w) => this.curve((x) => polyEval(w, x), { color: C.acc, width: 1 }));
    g.globalAlpha = 1;
    this.curve(TARGET, { color: C.muted, width: 1.75, dash: [5, 4] });
    this.polyline(gx.map((x, i) => [x, gbar[i]]), { color: C.acc, width: 2.75 });
    this.label("ḡ  (average of the " + K + " fits)", this.left + 6, this.top + 12, { color: C.acc });
    this.label("f", this.x(0.75), this.y(TARGET(0.75)) - 12, { align: "center" });
...
class SummaryPlot extends Plot {
  constructor(canvas, colors, yHi) {
    super(canvas, { width: SW, height: SH, pad: 36, padTop: 14, padRight: 12,
      xDomain: [0, MAXM], yDomain: [0, yHi], colors });
  }

  render({ bias2, variance, M }) {
    const C = this.C;
    this.grid({ yTicks: 4 }).axes();
    const total = bias2.map((b, i) => b + variance[i] + SIGMA * SIGMA);
    this.polyline(total.map((v, i) => [i, v]), { color: C.neg, width: 2.25 });
    this.polyline(bias2.map((v, i) => [i, v]), { color: C.pos, width: 2 });
    this.polyline(variance.map((v, i) => [i, v]), { color: C.acc, width: 2 });
    total.forEach((v, i) => this.dot(i, v, { color: C.neg, r: 2.5 }));
    bias2.forEach((v, i) => this.dot(i, v, { color: C.pos, r: 2.5 }));
    variance.forEach((v, i) => this.dot(i, v, { color: C.acc, r: 2.5 }));
    this.vline(M, { color: C.fg, width: 1.25, dash: [3, 3] });
    this.label("bias²", this.x(0.2), this.y(bias2[0]) - 8, { color: C.pos });
    this.label("variance", this.x(MAXM) - 4, this.y(Math.min(variance[MAXM], this.yDomain[1])) - 8,
      { align: "right", color: C.acc });
    this.label("bias² + variance + σ²", this.x(MAXM / 2), this.top + 12, { align: "center", color: C.neg });
    for (let m = 0; m <= MAXM; m += 3) this.label(String(m), this.x(m), this.bottom + 14, { align: "center" });
    this.label("M", (this.left + this.right) / 2, this.bottom + 28, { align: "center" });
    for (let k = 1; k <= 4; k++) {
...
export default function App() {
  ...
  const all = React.useMemo(
    ...
    [seed]);
  const cur = all[M];
  const bias2 = all.map((r) => r.bias2), variance = all.map((r) => Math.min(r.variance, 0.6));
  const yHi = 0.6;

  React.useEffect(() => {
    if (ref.current) new FitsPlot(ref.current, C).render(cur);
    if (sref.current) new SummaryPlot(sref.current, C, yHi).render({ bias2, variance, M });
  });

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px", color: C.fg }}>
      <h2 className="sr-only">Bias and variance of polynomial fits: forty fits of degree M to forty noisy samples of a sine, their average, and the bias-squared and variance curves against degree.</h2>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        ...
        <button type="button" style={buttonStyle(C, M === 1)} onClick={() => setM(1)}>M = 1</button>
        <button type="button" style={buttonStyle(C, M === 3)} onClick={() => setM(3)}>M = 3</button>
        <button type="button" style={buttonStyle(C, M === 9)} onClick={() => setM(9)}>M = 9</button>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "flex-start", justifyContent: "center" }}>
        ...
        <canvas ref={sref} style={{ flex: `1 1 ${SW}px`, maxWidth: SW, aspectRatio: `${SW} / ${SH}` }}
          aria-label={`Bias squared, variance and their sum against degree, currently at M = ${M}`} />
      </div>
      <p style={{ ...readoutStyle(C), margin: 0 }}>
        at M = {M}:  bias² = {cur.bias2.toFixed(3)}   variance = {cur.variance.toFixed(3)}   σ² = {(SIGMA * SIGMA).toFixed(3)}   sum = {(cur.bias2 + cur.variance + SIGMA * SIGMA).toFixed(3)}
      </p>
      <p style={{ ...labelStyle(C), margin: 0, lineHeight: 1.5 }}>
        Left: {K} samples of {N} noisy points each from the dashed target, a degree-M
        polynomial fitted to every one (faint), and their average ḡ (bold). Right: the same
        experiment at every degree. Bias² is the squared distance from ḡ to the target, averaged
        over x - what the hypothesis set cannot express however much data it sees. Variance is
        the squared scatter of the individual fits around ḡ - how much the answer depends on
        which sample you happened to draw. Walk M up: at 1 the fits agree and are all wrong
        together; at 9 their average is nearly right and no single one of them is. E_out is
        the sum of the two plus the noise floor σ², and its minimum is where the falling and
        rising curves cross over, not where either one is small. (The variance axis is
        clipped at 0.6; at M = 9 the true value is larger.)
      </p>
    </div>
  );
}
