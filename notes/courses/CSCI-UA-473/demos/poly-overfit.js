/* AUTO-GENERATED from poly-overfit.jsx by `npm run build:artifacts`. Do not edit. */
...
class FitPlot extends Plot {
  ...
  render({
    ...
    this.label("x", this.right, this.bottom + 15, {
      align: "right"
    });
    this.label("y", this.left - 6, this.top + 6, {
      align: "right"
    });
    return this;
  }
...
export default function App() {
  ...
  const {
    xs,
    ys
  } = React.useMemo(() => sineSample(10, SIGMA, rng(seed)), [seed]);
  const w = React.useMemo(() => polyFit(xs, ys, M), [xs, ys, M]);
  const ein = rmse(w, xs, ys),
    eout = eoutNoisy(w, SIGMA);
  React.useEffect(() => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sr-only"
  }, "Polynomial fits of increasing degree to ten noisy points from a sine, with in-sample and out-of-sample error."), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px",
  ...
  }, "New sample"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    style: buttonStyle(C, M === 9),
    onClick: () => setM(9)
  }, "Jump to M = 9")), /*#__PURE__*/React.createElement("canvas", {
    ref: ref,
    style: {
      width: "100%",
      maxWidth: W,
  ...
  }, "E_in = ", ein.toFixed(3), "    E_out = ", eout.toFixed(3)), /*#__PURE__*/React.createElement("p", {
    style: {
      ...labelStyle(C),
      margin: 0,
      lineHeight: 1.5
    }
  }, "Dashed grey is the target the data came from, the solid line is the fit, dots are the ten training points. Raising M always drives E_in down, because a richer hypothesis set can only fit the sample better. E_out does not follow: it bottoms out near M = 3 and then climbs. At M = 9 the curve passes through every point, E_in reaches zero, and the fit is worthless between them. Zero training error is not evidence of learning."));
}