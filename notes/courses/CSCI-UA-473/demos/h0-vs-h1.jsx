...
const W = 330, H = 250, NMAX = 20;

class LinesPlot extends Plot {
  constructor(canvas, colors) {
    super(canvas, { width: W, height: H, pad: 26, padTop: 16, padRight: 10,
      xDomain: [-1, 1], yDomain: [-2.5, 2.5], colors });
  }

  render({ exp, name, formula }) {
    const C = this.C, g = this.g;
    this.grid({ yTicks: 4, edges: false }).axes({ atY: 0, atX: 0 });
    ...
    g.fillStyle = C.acc; g.globalAlpha = 0.13;
    g.beginPath();
    exp.gbar.forEach((v, i) => {
      const px = this.x(this.gx[i]), py = this.y(v + exp.sd[i]);
    ...
    g.globalAlpha = 0.2;
    exp.fits.forEach(([a, b]) => this.curve((x) => a * x + b, { color: C.acc, width: 1, samples: 2 }));
    g.globalAlpha = 1;
    this.curve(SINPI, { color: C.muted, width: 1.75, dash: [5, 4] });
    this.polyline(this.gx.map((x, i) => [x, exp.gbar[i]]), { color: C.acc, width: 2.75 });

    this.label(name + ":  " + formula, this.left + 6, this.top + 12, { color: C.fg });
    this.label("sin(πx)", this.x(0.5), this.y(1) - 8, { align: "center" });
    this.label("x", this.right, this.y(0) + 14, { align: "right" });
    return this;
  }
}
...
export default function App() {
  ...
  const r0 = React.useRef(null), r1 = React.useRef(null);

  const T = Math.round(600000 / N);
  const exp = React.useMemo(() => linesExperiment(N, { T }, rng(seed * 101 + N)), [N, seed, T]);
  ...
  React.useEffect(() => {
    for (const [ref, k, name, formula] of [[r0, "h0", "H0", "h(x) = b"], [r1, "h1", "H1", "h(x) = ax + b"]]) {
      ...
      p.gx = exp.gx;
      p.render({ exp: exp[k], name, formula });
    }
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px", color: C.fg }}>
      <h2 className="sr-only">The sin of pi x example: constant fits and line fits to N points each, repeated over many datasets, with their average hypothesis, a one-standard-deviation band, and the bias and variance of each hypothesis set as N changes.</h2>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        <span style={labelStyle(C)}>points per dataset N</span>
        <input type="range" min="2" max={NMAX} step="1" value={N} onChange={(e) => setN(+e.target.value)}
          aria-label="Number of training points N" style={{ flex: "1 1 150px", minWidth: "130px", accentColor: C.acc }} />
        <span style={{ ...readoutStyle(C), margin: 0, minWidth: "3.5em" }}>N = {N}</span>
        <button type="button" style={buttonStyle(C, N === 2)} onClick={() => setN(2)}>N = 2 (slide)</button>
        <button type="button" style={buttonStyle(C, N === 4)} onClick={() => setN(4)}>N = 4</button>
      ...
      <p style={{ ...readoutStyle(C), margin: 0 }}>
        {line("h0", "H0")}<br />
        {line("h1", "H1")}<br />
        at N = {N}, {winner} wins ({fmt(Math.min(tot("h0"), tot("h1")))} against {fmt(Math.max(tot("h0"), tot("h1")))})
      </p>
      <p style={{ ...labelStyle(C), margin: 0, lineHeight: 1.5 }}>
        Each panel: sixty datasets' fits (faint), their average ḡ (bold), the band ḡ ± one
        standard deviation, and the dashed target. The numbers are over {T.toLocaleString("en-US")} datasets
        (the slide's 1.69 for H1's variance is a simulation estimate; the exact value is 1.68).
        Walk N up: bias hardly changes for either set, variance falls for both, and the
        lines overtake the constants at N = 4 without anything about f changing.
      </p>
    </div>
  );
}
