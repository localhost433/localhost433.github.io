...
export default function ErmObjectiveAnatomy() {
  ...
  return (
    ...
    >
      <FigureSvg viewBox="0 0 764 404" maxWidth={764}
        ...
        <TeX x={96} y={64} w={220} anchor="middle" size={19} fill={C.fg}
          tex="\arg\min\limits_{M\,\in\,\mathcal H}" />
        <TeX x={300} y={64} w={340} anchor="middle" size={19} fill={C.fg}
          tex="\frac{1}{N}\sum_{n=1}^{N}\Delta\big(y_n,\,M,\,x_n\big)" />
        <TeX x={462} y={64} w={40} anchor="middle" size={19} fill={C.muted} tex="+" />
        <TeX x={500} y={64} w={40} anchor="middle" size={19} fill={C.neg} tex="\lambda" />
        <TeX x={560} y={64} w={120} anchor="middle" size={19} fill={C.fg} tex="C(M)" />

        {TERMS.map((t, i) => (
          <g key={i}>
            <Arrow from={[t.ax, 96]} to={[t.ax, 126]} C={C} tone={t.tone} width={1.2} />
            <Lines x={t.lx} y={146} size={11} fill={tone[t.tone]} lines={t.label} />
          </g>
        ))}

        <line x1={30} y1={226} x2={734} y2={226} stroke={C.border} strokeWidth="1" />

        <text x={30} y={250} fontSize="12" fill={C.muted} fontStyle="italic">note 00 (Cho)</text>
        <text x={196} y={250} fontSize="12" fill={C.muted} fontStyle="italic">the lecture decks</text>
        <text x={392} y={250} fontSize="12" fill={C.muted} fontStyle="italic">what it is</text>
        {TRANSLATE.map(([a, b, c], i) => {
          const y = 280 + i * 24;
          return (
            <g key={i}>
              <TeX x={30} y={y} w={160} size={13} fill={C.fg} tex={a} />
              <TeX x={196} y={y} w={190} size={13} fill={C.acc} tex={b} />
              <text x={392} y={y} fontSize="12" fill={C.muted}>{c}</text>
            </g>
          );
        })}
      </FigureSvg>
