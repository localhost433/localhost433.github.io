...
const W = 470, H = 290, FW = 264, MAXM = 9, SIGMA = 0.2, NVAL = 10, NTEST = 200;
...
class UCurvePlot extends Plot {
  ...
  render({ ein, eout, eval_, M, picked }) {
    const C = this.C;
    this.grid({ yTicks: 4 });
    ...
    this.axes();
    this.polyline(eval_.map((v, i) => [i, v]), { color: C.acc, width: 1.75, dash: [5, 4] });
    this.polyline(eout.map((v, i) => [i, v]), { color: C.neg, width: 2.25 });
    this.polyline(ein.map((v, i) => [i, v]), { color: C.pos, width: 2.25 });
    eval_.forEach((v, i) => this.dot(i, v, { color: C.acc, r: 2.5, halo: true }));
    ...
    if (picked !== best) {
      this.label("validation picks " + picked, this.x(picked) + (picked > MAXM / 2 ? -6 : 6), this.top + 26,
        { align: picked > MAXM / 2 ? "right" : "left", color: C.acc });
    } else {
      this.label("= validation's pick", this.x(best) + (best > MAXM / 2 ? -6 : 6), this.top + 26,
        { align: best > MAXM / 2 ? "right" : "left", color: C.acc });
    }
    this.vline(M, { color: C.acc, width: 2 });

    this.label("E_in", this.right - 4, this.y(ein[MAXM]) - 8, { align: "right", color: C.pos });
    this.label("E_out", this.right - 4, this.y(eout[MAXM]) - 8, { align: "right", color: C.neg });
    this.label("E_val", this.right - 4, this.y(eval_[MAXM]) + 14, { align: "right", color: C.acc });
...
class FitPanel extends Plot {
  ...
  render({ xs, ys, vx, vy, w }) {
    ...
    vx.forEach((x, i) => this.dot(x, vy[i], { color: C.acc, r: 3, halo: false, ring: null }));
    xs.forEach((x, i) => this.dot(x, ys[i], { color: C.pos, r: 3.5 }));
...
export default function App() {
  ...
  const { ein, eout, eval_, etest, xs, ys, vx, vy } = React.useMemo(() => {
    const { xs: sx, ys: sy } = sineSample(10, SIGMA, rng(seed));
    const { xs: vx0, ys: vy0 } = sineSampleRandom(NVAL, SIGMA, rng(seed * 7919 + 11));
    const { xs: tx, ys: ty } = sineSampleRandom(NTEST, SIGMA, rng(seed * 104729 + 23));
    const ei = [], eo = [], ev = [], et = [];
    ...
    return { ein: ei, eout: eo, eval_: ev, etest: et, xs: sx, ys: sy, vx: vx0, vy: vy0 };
  }, [seed]);
  ...
  React.useEffect(() => {
    if (ref.current) new UCurvePlot(ref.current, C, yHi).render({ ein, eout, eval_, M, picked });
    if (fitRef.current) new FitPanel(fitRef.current, C).render({ xs, ys, vx, vy, w: wM });
  });

  const gap = eout[M] - ein[M];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px", color: C.fg }}>
      <h2 className="sr-only">In-sample, validation and out-of-sample error against polynomial degree, with the generalization gap shaded and the degree validation selects marked.</h2>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        ...
        <button type="button" style={buttonStyle(C, M === picked)} onClick={() => setM(picked)}>Pick M by validation</button>
      ...
      <p style={{ ...readoutStyle(C), margin: 0 }}>
        at M = {M}:  E_in = {ein[M].toFixed(3)}   E_val = {eval_[M].toFixed(3)}   E_out = {eout[M].toFixed(3)}   gap = {gap.toFixed(3)}
      </p>
      <p style={{ ...readoutStyle(C), margin: 0 }}>
        validation picks M = {picked}:  E_val = {eval_[picked].toFixed(3)}   E_test (fresh {NTEST} points) = {etest[picked].toFixed(3)}
        {etest[picked] > eval_[picked] ? "   — the validation number is optimistic" : ""}
      </p>
      <p style={{ ...labelStyle(C), margin: 0, lineHeight: 1.5 }}>
        The lower solid curve is E_in, the upper one is E_out, and the shaded area between
        them is the generalization gap - the only quantity Hoeffding bounds. Read the two
        ...
        tells you where to stand. The grey dashed line marks the degree that actually
        minimizes E_out; nothing you can compute from the training set alone points at it.
        The dashed curve is what you <em>can</em> compute: the error on ten held-out
        validation points, drawn hollow in the right panel. Press the button to let it
        choose M, then read its error at that degree against the fresh test sample. The
        validation error is usually the smaller of the two, because you chose the degree
        that made it small: that is selection bias, the M-bins argument one level up, and
        it is why the test set has to stay untouched until the choice is made.
      </p>
    </div>
  );
}
