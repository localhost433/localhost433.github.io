...
*Note:* If $X$ and $Y$ are independent, $E[XY] = E[X]E[Y]$, so $\text{Cov}(X, Y) = 0$. This recovers the property that variance is additive for independent variables. However, $\text{Cov}(X, Y) = 0$ does *not* imply independence.
...
$$\text{Var}(Y \mid X) = E[(Y - E[Y|X])^2 \mid X]$$
...
* **Intra-group variance ($E[\text{Var}(Y|X)]$):** The average variance within each group defined by $X$.
* **Inter-group variance ($\text{Var}(E[Y|X])$):** The variance of the group means.
