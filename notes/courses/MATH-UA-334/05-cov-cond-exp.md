...
*Note:* If $X$ and $Y$ are independent, $\E[XY] = \E[X]\E[Y]$, so $\Cov{X}{Y} = 0$. This recovers the property that variance is additive for independent variables. However, $\Cov{X}{Y} = 0$ does *not* strictly imply independence.
...
Correlation normalizes the covariance to be unitless and bounded between -1 and 1.
...
**Application (Random Sums - Wald's Equation):**
