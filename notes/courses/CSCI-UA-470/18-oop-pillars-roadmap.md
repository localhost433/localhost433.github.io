...
- UML and design patterns: reusable notations and designs, built mostly from inheritance, abstraction, and polymorphism. UML is developed in [UML & Use Case Diagrams](note.html?course=CSCI-UA-470&note=12-uml-use-case-diagrams), and each class-diagram relationship is translated back into Java in [From Class Diagram to Code](note.html?course=CSCI-UA-470&note=15-uml-to-code). The pattern catalog splits three ways — how objects are made ([Creational](note.html?course=CSCI-UA-470&note=19-creational-patterns)), how they are composed ([Structural](note.html?course=CSCI-UA-470&note=20-structural-patterns)), and how they talk (Behavioral, split across [note 21](note.html?course=CSCI-UA-470&note=21-behavioral-patterns) and [note 22](note.html?course=CSCI-UA-470&note=22-behavioral-patterns-ii)) — and every one of the twenty is one of these four pillars applied to a recurring problem.
...
- The process around all of it: which phase of a development cycle each of the above belongs to, who owns it, and what a design skill is worth once a model can write the code, are in [The SDLC, the Roles & Design in the AI Era](note.html?course=CSCI-UA-470&note=23-design-in-the-ai-era). That note is the frame; this one is the map.
