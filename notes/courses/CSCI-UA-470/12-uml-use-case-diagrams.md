...
**UML** (Unified Modeling Language) is the shared notation the **Analyst** and **Designer** use to capture a design and communicate it to everyone downstream. It is a *communication tool*, not a programming language, and it sits above the C++/Java implementation details covered in [note 11](note.html?course=CSCI-UA-470&note=11-jvm-runtime) and [note 14](note.html?course=CSCI-UA-470&note=14-java-cpp-systematic-comparison).
...
> Where this sits in the course: UML is a **notation for design**, applied on top of the OOP ideas summarized in [note 15](note.html?course=CSCI-UA-470&note=15-oop-pillars-roadmap). A use case diagram captures requirements from the outside (*what, not how*) before those requirements are turned into the classes, inheritance, and polymorphism built throughout the earlier notes.
