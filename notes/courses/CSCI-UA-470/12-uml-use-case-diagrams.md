...
UML (Unified Modeling Language) is the shared notation the analyst and designer use to capture a design and communicate it to everyone downstream. It is a communication tool, not a programming language, and it sits above the C++/Java implementation details covered in [the JVM note (L10–L11)](note.html?course=CSCI-UA-470&note=11-jvm-runtime) and [the Java/C++ comparison (L11)](note.html?course=CSCI-UA-470&note=17-java-cpp-systematic-comparison).
...
> Where this sits in the course: UML is a notation for design, applied on top of the OOP ideas summarized in [the pillars roadmap](note.html?course=CSCI-UA-470&note=18-oop-pillars-roadmap). A use case diagram captures requirements from the outside, what and not how, before those requirements are turned into the classes, inheritance, and polymorphism built throughout the earlier notes.
