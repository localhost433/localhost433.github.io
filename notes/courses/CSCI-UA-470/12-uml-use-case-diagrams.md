...
**UML** (Unified Modeling Language) is the shared notation the **Analyst** and **Designer** use to capture a design and communicate it to everyone downstream. It is a *communication tool*, not a programming language, and it sits above the C++/Java implementation details covered in [note 11](note.html?course=CSCI-UA-470&note=11-jvm-runtime) and [note 13](note.html?course=CSCI-UA-470&note=13-java-cpp-systematic-comparison).
...
UML diagrams split into three groups. The bold entry in each row is the one this note develops.
...
- **Structural** diagrams freeze the system and show its parts and how they are wired: the **class diagram** (types, fields, methods, and associations) is the one most tied to the OOP pillars in [note 14](note.html?course=CSCI-UA-470&note=14-oop-pillars-roadmap); the **object diagram** shows a concrete snapshot of instances; package/component/deployment zoom out to modules, binaries, and physical nodes.
...
- **Interaction** diagrams are a zoom-in on behavior: **sequence** (messages ordered top-to-bottom along lifelines), **collaboration** (the same messages numbered on an object graph), and **timing** (state against time).

The rest of this note is about the **use case diagram**, the L12 topic.

## Use case diagrams

A use case diagram answers "what should this system let people do?" from the outside.

- **Visualize the behavior** of the system.
- Serve as **a way to communicate** with clients and teammates.
- Describe what the system does **from the standpoint of an external observer** — again, *what, not how*.
...
- An actor need not be a person — an external system (a bank, a legacy service) is an actor too.
...
**1. Identify the actors** — the people or systems that will interact with the scenario. Guiding questions:

- Who is interested in / benefits from the system?
...
**2. Identify the use cases** — a summary of scenarios for a single task or goal. Guiding questions:

- What are the tasks of each actor?
- Will any actor create, store, change, remove, or read information?
- Which use cases create/store/change/remove/read that information?
...
**3. Draw the relation** — connect the actor to the use case with a communication line:
...
**Generalization** — the hollow-triangle arrow points from each child up to the parent (the same notation as class inheritance):

```artifact src=demos/use-case-generalization.jsx static
```

**Include** — a dashed arrow from each base to the step it always pulls in:

```artifact src=demos/use-case-include.jsx static
```

**Extend** — a dashed arrow from each optional extension back to the base it may add to:

```artifact src=demos/use-case-extend.jsx static
```

**Include vs. Extend in one line:** `include` = mandatory, factored-out common behavior that *always* runs as part of the base; `extend` = optional behavior that runs *only under certain conditions*, on a base that is already complete on its own.
...
- **Library System** — actors `Borrower (Member)` and `Librarian`. The borrower can `Borrow Book`, `Search for book`, `Return Book`, `List all Borrowings`; the librarian can `Add book`, `Remove Book`, `update Book`, `List all Borrowings`, and `Organize Books`. A refined version splits the use cases per actor and adds `<<extend>>` links (e.g. `Borrow Book` extends `Search for book`, `Return Book` extends `List all Borrowings`). This is the first diagram with real structure — two actors, a shared use case, and relations between use cases — so it is worth building up one piece at a time:
...
> Where this sits in the course: UML is a **notation for design**, applied on top of the OOP ideas summarized in [note 14](note.html?course=CSCI-UA-470&note=14-oop-pillars-roadmap). A use case diagram captures requirements from the outside (*what, not how*) before those requirements are turned into the classes, inheritance, and polymorphism built throughout the earlier notes.
