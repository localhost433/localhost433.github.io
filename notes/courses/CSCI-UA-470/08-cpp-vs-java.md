...
- **C++** compiles all the way down to machine code for one specific platform. You must recompile (and produce a different binary -- `a.out` on macOS/Linux, `.exe` on Windows) for each target.
- **Java** compiles once to **bytecode**, then the **JVM** on each machine (macOS, Linux, Windows) interprets it -- *write once, run anywhere*.
...
**Java** -- default is `none` (package-private):
...
Both leave the object on the heap with a stack variable pointing at it. The key Java consequence is that **assignment copies the reference, not the object**:
