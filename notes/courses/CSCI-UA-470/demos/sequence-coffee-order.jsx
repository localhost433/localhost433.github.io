...
export default sequenceDiagram({
  ...
  messages: [
    { from: "cust", to: "cashier", label: "place_order(order_details)", kind: "sync" },   // 0
    { from: "cashier", to: "cust", label: "price", kind: "return" },                       // 1
    { from: "cust", to: "cashier", label: "get_payment(price)", kind: "sync" },            // 2
    { from: "cashier", to: "barista", label: "prepare(order_details)", kind: "sync" },     // 3
    { from: "barista", to: "recep", label: "deliver(order)", kind: "sync" },               // 4
    { from: "recep", to: "recep", label: "checkQuality(order)", kind: "sync", self: true },// 5
    { from: "recep", to: "cust", label: "order", kind: "return" },                         // 6
  ],
  activations: [
