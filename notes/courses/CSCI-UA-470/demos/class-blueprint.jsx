...
const c1Sections = [{ rows: ['color = "red"', "radius = 2.0"] }];
const c2Sections = [{ rows: ['color = "blue"', "radius = 3.0"] }];

const BP = { x: 28, y: 44, w: 184, h: diagramCardHeight(blueprintSections) };   // 122
const C1 = { x: 388, y: 34, w: 168, h: diagramCardHeight(c1Sections) };          // 74
const C2 = { x: 388, y: 132, w: 168, h: diagramCardHeight(c2Sections) };         // 74
...
export default function ClassBlueprint() {
  return (
    <DiagramSvg viewBox="0 0 600 250" maxWidth={600}
      ...
      <DiagramEdge from={{ x: bpRight, y: bpMidY - 17 }} to={{ x: instLeft, y: c1MidY }} label="instance of" />
      <DiagramEdge from={{ x: bpRight, y: bpMidY + 17 }} to={{ x: instLeft, y: c2MidY }} label="instance of" />

      <DiagramCard x={BP.x} y={BP.y} w={BP.w} title="Circle" sections={blueprintSections} sub={0} />

      <DiagramCard x={C1.x} y={C1.y} w={C1.w} title="c1 : Circle" sections={c1Sections} sub={2} />
      <DiagramCard x={C2.x} y={C2.y} w={C2.w} title="c2 : Circle" sections={c2Sections} sub={2} />
