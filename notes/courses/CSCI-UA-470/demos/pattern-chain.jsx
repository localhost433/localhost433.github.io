...
const T = patternTree({
  place: "above", contextW: 208, gapY: 44, edge: "depend", edgeLabel: "handles",
  context: { title: "Application", sections: [{ rows: ["- student : String", "- request : String"] }] },
  parent: { title: "Handler", abstract: true, sections: [
    { rows: ["- nextHandler : Handler"] },
    { rows: [ab("+ handle(a)")] },
  ]},
  children: ["Secretary", "Chair", "Dean", "Assistant"].map((t) => ({
    title: t, sections: [{ rows: ["+ handle(a)"] }],
  })),
  cardW: 138, gap: 16,
...
if (!dean.handle(a)) assistant.handle(a);`,
  ...
  good: {
    ...
    node: (
      <g>
        {T.node}
        <SvgCode x={14} y={T.height - 4} lines={BODY} title="Handler.handle(a)" />
      </g>
    ),
  },
  client: {
...
c.handle(a);           // starts mid-ladder; the secretary is last resort`,
  ...
  caption: {
    cols: [
      { tag: "sender", kind: "cpp", children: <>Knows <strong>one</strong> handler and nothing about the rest. It does not know who will answer, how many were asked, or whether anyone did.</> },
      { tag: "chain", kind: "int", children: <>Each handler knows only its <strong>successor</strong>. Insert, remove, or reorder by assigning a field — no class is edited and no subclass is added.</> },
    ],
