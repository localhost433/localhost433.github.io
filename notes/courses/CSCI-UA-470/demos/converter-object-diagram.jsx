...
const mainRows = [{ rows: ['b1 = "Kg → Lb"', 'b2 = "Cm → Inch"', 'b3 = "Quit"'] }];
const kgRows = [{ rows: ['in  = "5.0"', 'out = "11.02"', "c   = c"] }];
const cmRows = [{ rows: ['in  = "12.0"', 'out = "4.72"', "c   = c"] }];
const convRows = [{ rows: ["(no attributes)"] }];

const B = { x: 20, y: 104, w: 190, h: diagramCardHeight(mainRows) };    // 92
const K1 = { x: 284, y: 20, w: 200, h: diagramCardHeight(kgRows) };     // 92
const K2 = { x: 284, y: 190, w: 200, h: diagramCardHeight(cmRows) };    // 92
const C = { x: 566, y: 122, w: 190, h: diagramCardHeight(convRows) };   // 56
...
export default function ConverterObjectDiagram() {
  return (
    <DiagramSvg viewBox="0 0 780 330" maxWidth={700}
      ...
      <UmlLink from={{ x: bR, y: 128 }} to={{ x: K1.x, y: 66 }} kind="assoc" />
      <UmlLink from={{ x: bR, y: 176 }} to={{ x: K2.x, y: 236 }} kind="assoc" />
      <UmlLink from={{ x: kR, y: 66 }} to={{ x: C.x, y: 138 }} kind="assoc" />
      <UmlLink from={{ x: kR, y: 236 }} to={{ x: C.x, y: 162 }} kind="assoc" />

      <DiagramCard x={B.x} y={B.y} w={B.w} title="b : MainGUI" sections={mainRows} sub={0} underline />
      <DiagramCard x={K1.x} y={K1.y} w={K1.w} title="k1 : KgLbGUI" sections={kgRows} sub={2} underline />
      <DiagramCard x={K2.x} y={K2.y} w={K2.w} title="k2 : CmInchGUI" sections={cmRows} sub={2} underline />
      <DiagramCard x={C.x} y={C.y} w={C.w} title="c : Converter" sections={convRows} sub={1} underline />

      <text x={390} y={312} textAnchor="middle"
