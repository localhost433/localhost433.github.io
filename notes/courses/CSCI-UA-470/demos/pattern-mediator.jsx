...
const T = patternTree({
  contextW: 262, gapX: 46, edge: "aggregate", edgeLabel: "colleagues",
  context: { title: "Mediator", sections: [
    { rows: ["- colleagues : List<Colleague>"] },
    { rows: ["+ broadcast(sender, msg)"] },
  ]},
  parent: { title: "«interface» Colleague", abstract: true,
    sections: [{ rows: [ab("+ receiveNotification(msg)")] }] },
  children: ["Colleague-1", "Colleague-2", "Colleague-3"].map((t) => ({
    title: t, sections: [{ rows: ["+ receiveNotification(msg)"] }],
  })),
  relation: "implements",
  cardW: 202, gap: 14,
});
...
// Adding a fifth means editing the four that already exist.`,
  ...
  good: {
    ...
    node: (
      <g>
        {T.node}
        <SvgCode x={14} y={T.height - 4} lines={BODY} title="Mediator.broadcast(sender, msg)" />
      </g>
    ),
  },
  client: {
...
room.broadcast(c1, "the build is red");   // c2 and c3 hear it; c1 does not`,
  ...
  caption: {
    cols: [
      { tag: "before", kind: "cpp", children: <>Many-to-many: <strong>n(n−1)</strong> references, each peer maintaining its own copy of who exists.</> },
      { tag: "after", kind: "int", children: <>Hub-and-spoke: <strong>n</strong> references, all held by one object whose entire job is knowing the membership.</> },
    ],
