...
import {
  PatternFigure, KnobBar, treeLayout, ClassTree, DiagramCard,
  UmlLink, SvgCode, svgCodeSize, diagramCardHeight, ab,
} from "@course";
...
const leaf = (title, attrs, ops) => ({
  title,
  sections: attrs ? [{ rows: attrs }, { rows: ops }] : [{ rows: ops }],
});
...
const CASTS = {
  shapes: {
    label: "Shapes",
    elementType: "Shape",
    visitorParent: { title: "Visitor", abstract: true, sections: [{ rows: [ab("visit(Shape)")] }] },
    visitors: ["Rotator", "Resizer", "Flipper"].map((t) => leaf(t, null, ["visit(Shape)"])),
    elementParent: { title: "Shape", abstract: true,
      sections: [{ rows: ["color"] }, { rows: [ab("accept(Visitor)")] }] },
    elements: [
      leaf("Circle", ["radius"], ["accept(Visitor)"]),
      leaf("Rectangle", ["width", "length"], ["accept(Visitor)"]),
      leaf("Triangle", ["base", "height"], ["accept(Visitor)"]),
    ],
    badParent: { title: "Shape", abstract: true,
      sections: [{ rows: ["color"] }, { rows: [ab("rotate()"), ab("resize()")] }] },
    badElements: [
      leaf("Circle", ["radius"], ["rotate()", "resize()"]),
      leaf("Rectangle", ["width", "length"], ["rotate()", "resize()"]),
      leaf("Triangle", ["base", "height"], ["rotate()", "resize()"]),
    ],
...
// adding a Flipper touches no Shape class at all`,
  ...
  hospital: {
    label: "Hospital",
    elementType: "Patient",
    visitorParent: { title: "Visitor", abstract: true, sections: [{ rows: [ab("visit(Patient)")] }] },
    visitors: ["Physician", "Nurse", "Surgeon"].map((t) => leaf(t, null, ["visit(Patient)"])),
    elementParent: { title: "Patient",
      sections: [{ rows: ["name", "age"] }, { rows: ["accept(Visitor)"] }] },
    elements: [],
    badParent: { title: "Patient",
      sections: [{ rows: ["name", "age"] }, { rows: ["examine()", "operate()", "medicate()"] }] },
    badElements: [],
...
const KNOBS = [{
  id: "cast", label: "cast",
  options: [
    { value: "shapes", label: "Shapes · Rotator / Resizer" },
    { value: "hospital", label: "Hospital · Patient" },
  ],
}];
...
function badHalf(c) {
  if (c.badElements.length) {
    const T = treeLayout({ cx: 0, topY: PAD, cardW: CARD_W, gap: GAP, forkGap: FORK,
      parent: c.badParent, children: c.badElements });
    const dx = PAD - T.left;
    const w = Math.round(T.right + dx + PAD), h = Math.round(T.bottom + PAD);
    return {
      width: w, height: h, viewBox: `0 0 ${w} ${h}`, maxWidth: 560, note: c.badNote,
      ...
      node: <g transform={`translate(${dx}, 0)`}><ClassTree layout={T} /></g>,
    };
  }
  const h0 = diagramCardHeight(c.badParent.sections, { title: true });
  const w = CARD_W + 40 + 2 * PAD, h = h0 + 2 * PAD;
  return {
    width: w, height: h, viewBox: `0 0 ${w} ${h}`, maxWidth: 300, note: c.badNote,
    ariaLabel: "A single Patient class carrying name and age plus examine, operate and medicate. The diagram is crossed out.",
    node: <DiagramCard x={PAD} y={PAD} w={CARD_W + 40} title={c.badParent.title} sections={c.badParent.sections} neutral />,
  };
}
...
function goodHalf(c) {
  ...
  return {
    width: W, height: H, viewBox: `0 0 ${W} ${H}`, maxWidth: 880,
    ...
    node: (
      <g>
        <g transform={`translate(${aShift}, ${aDy})`}><ClassTree layout={A} /></g>
        {hasTree ? (
          <g transform={`translate(${bShift}, ${bDy})`}><ClassTree layout={B} /></g>
        ) : (
          <DiagramCard x={bX} y={PAD + bDy} w={bW}
            title={c.elementParent.title} sections={c.elementParent.sections} neutral />
        )}

        <UmlLink kind="depend"
          from={{ x: bParentLeft, y: axis - 11 }} to={{ x: aParentRight, y: axis - 11 }} />
        <UmlLink kind="depend"
          from={{ x: aParentRight, y: axis + 11 }} to={{ x: bParentLeft, y: axis + 11 }} />

        <SvgCode x={codeX} y={codeY} lines={ACCEPT} title={acceptTitle} />
  ...
  return (
    <div className="mm-scene">
      <div className="mm-scene__title" data-artifact-title>
        Visitor — the operation leaves the hierarchy
      </div>
      <KnobBar knobs={KNOBS} value={{ cast }} onChange={(_, v) => setCast(v)} />
      <PatternFigure
        intent="[Add additional behavior to an entity without changing its structure]"
        bad={badHalf(c)}
        good={goodHalf(c)}
        goodTag={`the pattern · ${c.label}`}
        client={{ code: c.client, lang: "java", label: "client code" }}
        caption={{
          cols: c.cols,
