...
const code = `int  a[4] = {3, 1, 4, 1};
int* q    = a;
int* arr  = new int[4];
delete[] arr;`;
const A = hl => stack("a", "int[4]", "", {
  id: "a",
  hl,
  fields: [{
    name: "a[0]",
...
const Q = hl => stack("q", "int*", "&a[0]", {
  id: "q",
  to: "a",
  hl
});
const ARR = hl => stack("arr", "int*", "0x…", {
  id: "arr",
  to: "harr",
  hl
});
const HARR = rec => heap("", "int[4]", "", {
  id: "harr",
  reclaimed: rec,
  fields: [{
    name: "[0]",
...
const steps = [{
  ...
  caption: {
    cpp: "`int* arr = new int[4];` puts an 8-byte **pointer** on the stack aimed at a 4-int block on the **heap**. The four ints are **uninitialized** (indeterminate `?`) — `new int[4]` default-initializes them; only `new int[4]{}` (or `new int[4]()`) would zero them.",
...
export default scene({
  title: "Arrays: stack vs heap, and the array-to-pointer decay",
  code,
  steps
});