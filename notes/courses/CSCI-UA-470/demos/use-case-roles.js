...
const bx1 = cx(umlIdx[0]) - BW / 2,
  bx2 = cx(umlIdx[umlIdx.length - 1]) + BW / 2;
...
export default function UseCaseRoles() {
  return /*#__PURE__*/React.createElement(DiagramSvg, {
  ...
  }, ROLES.slice(1).map((_, i) => /*#__PURE__*/React.createElement(DiagramEdge, {
    key: i,
    from: {
      x: cx(i) + BW / 2,
      y: CY
  ...
  })), ROLES.map((r, i) => /*#__PURE__*/React.createElement(DiagramBox, {
    key: i,
    cx: cx(i),
    cy: CY,
    w: BW,
    ...
    d: `M ${bx1} ${brY} L ${bx1} ${brY + 6} L ${bx2} ${brY + 6} L ${bx2} ${brY}`,
    style: {
      fill: "none",
      stroke: "var(--seg-global-tx)",
      strokeWidth: 1.4
    }
    ...
    textAnchor: "middle",
    style: {
      fill: "var(--seg-global-tx)",
      fontSize: 11,
      fontWeight: 700,
      fontFamily: "system-ui, sans-serif"
    }
  }, "author & read the UML model"));
}