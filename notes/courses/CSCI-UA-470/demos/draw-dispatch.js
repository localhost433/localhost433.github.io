...
const codeV2 = `void drawAll(Shape[] shapes) {
    for (Shape s : shapes) {
...
const codeV2Pent = `void drawAll(Shape[] shapes) {
    for (Shape s : shapes) {
        if (s.type == CIRCLE) drawCircle(s);
        else if (s.type == RECT) drawRect(s);
        else if (s.type == TRI) drawTri(s);
...
const codeV3 = `void drawAll(Shape[] shapes) {
    for (Shape s : shapes)
        s.draw();
}`;
...
const DRAW_RECT = hl => text("drawRect", "fn", "g.rect(s.x,s.y,s.w,s.h)", {
  id: "h_rect",
  hl
});
const DRAW_TRI = hl => text("drawTri", "fn", "g.poly(s,3)", {
  id: "h_tri",
  hl
});
const DRAW_PENT = hl => text("drawPent", "fn", "g.poly(s,5)", {
  id: "h_pent",
  hl
});

// v3: each subclass's own draw() body lives once in the Code segment.
const CIRCLE_DRAW = hl => text("Circle.draw", "fn", "g.oval(x,y,r)", {
  id: "d_circle",
  hl
});
const RECT_DRAW = hl => text("Rectangle.draw", "fn", "g.rect(x,y,w,h)", {
  id: "d_rect",
  hl
});
const TRI_DRAW = hl => text("Triangle.draw", "fn", "g.poly(3)", {
  id: "d_tri",
  hl
});
const PENT_DRAW = hl => text("Pentagon.draw", "fn", "g.poly(5)", {
  id: "d_pent",
  hl
});

// v3: one method table per class. The `draw` slot points at that class's body.
const MT_CIRCLE = hl => glob("Circle methods", "method table", "", {
  id: "mt_circle",
  hl,
  fields: [{
    name: "draw",
...
const MT_RECT = hl => glob("Rectangle methods", "method table", "", {
  id: "mt_rect",
  hl,
  fields: [{
    name: "draw",
...
const MT_TRI = hl => glob("Triangle methods", "method table", "", {
  id: "mt_tri",
  hl,
  fields: [{
    name: "draw",
...
const MT_PENT = hl => glob("Pentagon methods", "method table", "", {
  id: "mt_pent",
  hl,
  fields: [{
    name: "draw",
...
const steps = [{
  ...
  cells: [stack("s", "Shape", "", {
    ...
    hl: true
  }), MT_CIRCLE(), MT_RECT(), MT_TRI(), CIRCLE_DRAW(), RECT_DRAW(), TRI_DRAW()],
  caption: {
  ...
  cells: [stack("s", "Shape", "", {
    ...
    hl: true
  }), MT_CIRCLE(true), MT_RECT(), MT_TRI(), CIRCLE_DRAW(true), RECT_DRAW(), TRI_DRAW()],
  caption: {
  ...
  cells: [stack("s", "Shape", "", {
    ...
    hl: true
  }), MT_CIRCLE(), MT_RECT(), MT_TRI(), MT_PENT(true), CIRCLE_DRAW(), RECT_DRAW(), TRI_DRAW(), PENT_DRAW(true)],
  caption: {
