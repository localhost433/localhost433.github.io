...
const role = (title, kids) => ({
  parent: { title, abstract: true, sections: [{ rows: [ab("+ run()")] }] },
  children: kids.map((k) => ({ title: k, sections: [{ rows: ["+ run()"] }] })),
});
...
p1.receiveSMS();`,
  ...
  command: {
    ...
    context: { title: "Soldier", sections: [
...
s1.execute();                  // queued, logged, or handed on`,
    cols: [
      { tag: "who picks", kind: "cpp", children: <>Whoever <strong>issues</strong> the request — and the issuer and the receiver need never meet. The command object travels between them.</> },
      { tag: "what varies", kind: "int", children: <>The <strong>request itself becomes an object</strong>, so it can be put in a list, replayed, queued, or undone. Neither neighbour aims at that.</> },
    ],
  },
};
...
const KNOBS = [{
  id: "cast", label: "cast",
  options: [
    { value: "strategy", label: "Strategy · Team" },
    { value: "state", label: "State · Phone" },
    { value: "command", label: "Command · Soldier" },
  ],
}];
...
export default function PatternTrio() {
  ...
  const T = patternTree({
    contextW: 232, gapX: 50, edge: "assoc", edgeLabel: c.edgeLabel,
    context: c.context, parent: c.parent, children: c.children,
    cardW: 138, gap: 18,
  });
  T.maxWidth = 760;
  ...
  return (
    <div className="mm-scene">
      <div className="mm-scene__title" data-artifact-title>
        Strategy, State, Command — one picture, three meanings
      </div>
      <KnobBar knobs={KNOBS} value={{ cast }} onChange={(_, v) => setCast(v)} />
      <PatternFigure
        intent={c.intent}
        bad={{ code: c.bad, lang: "java", note: c.badNote }}
        good={T}
