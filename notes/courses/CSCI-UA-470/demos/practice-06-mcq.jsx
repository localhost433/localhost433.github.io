  ...
  questions: [
    {
      ...
      figure: { code: "struct Person  { virtual void intro(){ cout << \"person\";  } };\nstruct Student : Person { void intro(){ cout << \"student\"; } };\n\nStudent s1;\nPerson* ptr = &s1;\nptr->intro();        // ?", lang: "cpp" },
      choices: [
        { text: "\"student\" — `virtual` moves the choice to run time, so the object's actual type wins", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "object → **vptr** → **vtable** → function; the first virtual adds one hidden 8-byte `vptr` to **every object**, while the vtable is shared **one per class**", correct: true },
    ...
    },
    {
      ...
      figure: { code: "class Person { public: virtual void intro() = 0; };", lang: "cpp" },
      choices: [
        { text: "At least one **pure virtual** (`= 0`) method makes it abstract: you can declare **pointers/references** to it but cannot create **objects**", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "No — an inherited pure virtual left unimplemented keeps `Student` **abstract** too; only a subclass that implements *every* pure virtual becomes instantiable", correct: true },
    ...
    },
    {
      ...
      figure: { code: "struct Person  { string name; };\nstruct Teacher : Person {};\nstruct Student : Person {};\nstruct TA : Teacher, Student {};\nTA t;\nt.name;   // ?", lang: "cpp" },
      choices: [
        { text: "`TA` contains **two** separate `Person` subobjects, so `name` is **ambiguous** — which copy? (`Person* p = &t;` is ambiguous too)", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "**Virtual inheritance** — `Teacher`/`Student` derive `virtual public Person`; the one `Person` is shared, reached through a hidden `vbptr` with a run-time offset", correct: true },
