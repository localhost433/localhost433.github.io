...
const steps = [
  {
    key: "generalization", label: "generalization", kind: "generalize",
    top: cls("Person", [], []), bottom: cls("Employee", [], []),
  ...
  {
    key: "realization", label: "realization", kind: "realize",
    top: { title: "«interface» Drawable", abstract: true, sections: [{ rows: [] }] },
    bottom: cls("Shape", [], []),
  ...
  {
    key: "association", label: "association", kind: "assoc", horizontal: true,
    top: cls("Person", [], []), bottom: cls("Address", [], []),
  ...
  {
    key: "aggregation", label: "aggregation", kind: "aggregate", horizontal: true,
    top: cls("Manager", [], []), bottom: cls("Worker", [], []),
  ...
  {
    key: "composition", label: "composition", kind: "compose", horizontal: true,
    top: cls("Person", [], []), bottom: cls("Hand", [], []),
  ...
  {
    key: "dependency", label: "dependency", kind: "depend", horizontal: true,
    top: cls("LotteryTicket", ["+ num : int"], ["+ someMethod()"]), bottom: cls("Random", [], []),
...
function StepDiagram({ step }) {
  ...
  if (step.horizontal) {
    const y = 40, x1 = 20, x2 = 320;
    const midY = y + Math.max(topH, botH) / 2;
    return (
      <DiagramSvg viewBox="0 0 540 170" maxWidth={560}
        ariaLabel={`UML ${step.label}: ${step.top.title} ${step.label === "dependency" ? "depends on" : "linked to"} ${step.bottom.title}.`}>
        <UmlLink from={{ x: x1 + CARD_W, y: midY }} to={{ x: x2, y: midY }} kind={step.kind} />
        <DiagramCard x={x1} y={y} w={CARD_W} title={step.top.title} sections={step.top.sections}
          abstract={step.top.abstract} sub={0} />
        <DiagramCard x={x2} y={y} w={CARD_W} title={step.bottom.title} sections={step.bottom.sections} sub={2} />
      </DiagramSvg>
    );
  }
  // vertical: child below, arrow UP to the parent (generalization / realization)
  const x = 175, topY = 16, botY = topY + topH + 56;
  const H = botY + botH + 16;
  return (
    <DiagramSvg viewBox={`0 0 540 ${H}`} maxWidth={560}
      ...
      <UmlLink from={{ x: x + CARD_W / 2, y: botY }} to={{ x: x + CARD_W / 2, y: topY + topH }} kind={step.kind} />
      <DiagramCard x={x} y={topY} w={CARD_W} title={step.top.title} sections={step.top.sections}
        abstract={step.top.abstract} sub={0} />
      <DiagramCard x={x} y={botY} w={CARD_W} title={step.bottom.title} sections={step.bottom.sections} sub={2} />
    </DiagramSvg>
  );
}
...
export default function UmlCodeRelationships() {
  ...
  return (
    <div className="mm-scene">
      <div className="mm-scene__title" data-artifact-title>Six relationships, six shapes of Java</div>

      <StepDiagram step={step} />
      <CodeBlock code={step.code} lang="java" />

      <div className="mm-scene__caption mm-scene__caption--struct">
        <p className="mm-cap-row">
          <span className="mm-cap-tag mm-cap-tag--java">{step.label.toUpperCase()}</span>
          <span className="mm-cap-txt">{renderInline(step.caption)}</span>
        </p>
      </div>
