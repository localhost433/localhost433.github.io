...
const PP = part("(person)", "base part", [{
  name: "name",
  type: "string",
  size: 32,
  value: '"no name"'
}], "pp");
const SP = part("(student)", "part", [{
  name: "id",
  type: "int",
  value: "0"
}], "sp");
const GP = part("(gradStudent)", "part", [{
  name: "researchTopic",
  type: "string",
  size: 32,
  value: '"no topic"'
}], "gp");
const out = ladder(["person constructed", "student constructed", "gradStudent constructed", "==============================", "gradStudent destroyed", "student destroyed", "person destroyed"]);
const steps = [{
  line: [4, 5],
  cells: [PP(true)],
  outputs: out(1),
  ...
  line: [17, 18],
  cells: [PP(), SP(true, true), GP(false, true)],
  outputs: out(6),
  caption: {
  ...
  line: [7, 8],
  cells: [PP(true, true), SP(false, true), GP(false, true)],
  outputs: out(7),
  caption: {
...
const asmMap = {
  4: [1, 3],
  // person ctor: label + ret (leaf, no base call)
  14: [7, 8, 10],
  // student ctor: label + call person::person + ret
  24: [15, 16, 18],
  // gradStudent ctor: label + call student::student + ret
  34: [23, 24, 26, 27],
  // main: label + both ctor/dtor calls + ret
  27: [19, 21, 22],
  // ~gradStudent: label + call student::~student + ret
  17: [11, 13, 14],
  // ~student: label + call person::~person + ret
  7: [4, 6] // ~person: label + ret (leaf, no base call)
};
const asmLabel = "x86-64 · Intel (idealized)";
export default scene({
  title: "L05 — construction & destruction order (person -> student -> gradStudent)",
  code,
  steps,
  asm,
  asmMap,
