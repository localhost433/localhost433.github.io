...
const guiSections = [
  { rows: [
    "- btnAdd : JButton", "- btnDelete : JButton", "- btnExit : JButton",
    "- lblTitle : JLabel", "- txtTitle : JTextField", "- txtPassCode : JTextField",
    "- gui_list : JList", "- lstPasswords : DefaultListModel", "- contentPane : JPanel",
  ] },
  { rows: [
    "+ addNew(title, passcode) : void", "+ add_to_list(p) : void",
    "+ load_passwords() : void", "+ remove_from_list(pid) : void", "+ delete(pid) : void",
  ] },
];
const pwdSections = [
...
const GUI = { x: 24, y: 26, w: 268, h: diagramCardHeight(guiSections) };
const PWD = { x: 470, y: 40, w: 214, h: diagramCardHeight(pwdSections) };
const FM = { x: 470, y: 176, w: 246, h: diagramCardHeight(fmSections) };

export default function PkClassDiagram() {
  const H = Math.max(GUI.y + GUI.h, FM.y + FM.h) + 40;
  return (
    <DiagramSvg viewBox={`0 0 740 ${H}`} maxWidth={720}
      ...
      <UmlLink from={{ x: GUI.x + GUI.w, y: 78 }} to={{ x: PWD.x, y: 78 }} kind="aggregate" />
      <UmlLink from={{ x: GUI.x + GUI.w, y: 214 }} to={{ x: FM.x, y: 214 }} kind="depend" />

      <DiagramCard x={GUI.x} y={GUI.y} w={GUI.w} title="MainGUI" sections={guiSections} sub={0} />
      <DiagramCard x={PWD.x} y={PWD.y} w={PWD.w} title="Password" sections={pwdSections} sub={2} />
      <DiagramCard x={FM.x} y={FM.y} w={FM.w} title="FileManager" sections={fmSections} sub={3} />

      <text x={380} y={66} textAnchor="middle" style={{ fill: "var(--mm-muted)", fontSize: 10.5, fontStyle: "italic" }}>
        holds the list of passwords
      </text>
      <text x={380} y={202} textAnchor="middle" style={{ fill: "var(--mm-muted)", fontSize: 10.5, fontStyle: "italic" }}>
        uses to read / save
      </text>
      <text x={370} y={H - 10} textAnchor="middle" style={{ fill: "var(--mm-muted)", fontSize: 11 }}>
