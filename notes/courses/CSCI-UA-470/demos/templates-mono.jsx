...
const MI = (hl) => text("max<int>",    "T = int",    "a > b ? a : b", { id: "mi", hl });
const MD = (hl) => text("max<double>", "T = double", "a > b ? a : b", { id: "md", hl });
const MS = (hl) => text("max<string>", "T = string", "a > b ? a : b", { id: "ms", hl });

// the locals each call's result lands in, on the stack (declaration order).
const I = (hl) => stack("i", "int",    "7",    { id: "i", hl });
const D = (hl) => stack("d", "double", "9.5",  { id: "d", hl });
const X = (hl) => stack("x", "string", '"ab"', { id: "x", size: 32, hl });
const Y = (hl) => stack("y", "string", '"cd"', { id: "y", size: 32, hl });
const S = (hl) => stack("s", "string", '"cd"', { id: "s", size: 32, hl });
const J = (hl) => stack("j", "int",    "8",    { id: "j", hl });
...
export default scene({ title: "Monomorphization: one template, a separate compiled function per type", code, steps });
