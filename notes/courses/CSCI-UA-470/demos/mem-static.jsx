...
const A = (hl) => circle("a", ['"red"', "2.0"], { hl });
const B = (hl) => circle("b", ['"blue"', "3.0"], { hl });

const COUNT  = (hl) => glob("Circle::counter", "static int", "2", { id: "count", hl });
const RESET  = (hl) => text("Circle::resetCounter()", "static fn", "counter = 0", { id: "reset", hl });
const NLOCAL = (hl) => glob("nextId()::n", "static int", "2", { id: "slocal", hl });
const GUARD  = (hl) => glob("guard(n)", "byte", "1", { id: "guard", hl });
const FCOUNT = (hl) => glob("fileCounter", "static int", "0", { id: "fcount", hl });
...
const asmMap = {
  19: [9, 10, 23, 24, 25, 26],  // Circle a, b;  -> Circle::Circle bumps counter, called twice in main
  9:  [12, 13, 14],             // static void resetCounter()  -> its body (writes counter, no this)
  14: [15, 16, 18, 19, 20],     // static int n = 0;  -> nextId: guard check + load/inc/store
  1:  [1, 2],                   // static int fileCounter  -> data symbol + .long
};

const asmLabel = "x86-64 · Intel (idealized)";

export default scene({ title: "Where static lives: one shared copy, outside every object", code, steps, asm, asmMap, asmLabel });
