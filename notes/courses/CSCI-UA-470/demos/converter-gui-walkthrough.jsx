...
const mainSections = [{ rows: ["[  Kg → Lb   ]", "[  Cm → Inch ]", "[  Quit      ]"] }];
const kgSections = [
...
const MAIN = { x: 20, y: 62, w: 200, h: diagramCardHeight(mainSections) };   // 92
const KG = { x: 290, y: 10, w: 250, h: diagramCardHeight(kgSections) };      // 104
const CM = { x: 290, y: 160, w: 250, h: diagramCardHeight(cmSections) };     // 104

const mainRight = MAIN.x + MAIN.w;   // 220

export default function ConverterGuiWalkthrough() {
  return (
    <DiagramSvg viewBox="0 0 570 300" maxWidth={620}
      ...
      <DiagramEdge from={{ x: mainRight, y: 92 }} to={{ x: KG.x, y: 62 }} label="opens" dashed />
      <DiagramEdge from={{ x: mainRight, y: 122 }} to={{ x: CM.x, y: 212 }} label="opens" dashed />

      <DiagramCard x={MAIN.x} y={MAIN.y} w={MAIN.w} title="Unit Converter" sections={mainSections} sub={0} />
      <DiagramCard x={KG.x} y={KG.y} w={KG.w} title="Kg → Lb" sections={kgSections} sub={2} />
      <DiagramCard x={CM.x} y={CM.y} w={CM.w} title="Cm → Inch" sections={cmSections} sub={2} />

      <text x={285} y={288} textAnchor="middle"
