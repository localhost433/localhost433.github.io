...
const vtable = (body, hl) => stack("vt", "Circle vtable", "", { id: "vt", hl, fields: [
  ...
  steps: [
    {
      ...
      caption: { java: "`s` is declared `Shape` but references a `new Circle()`; the object carries a **class pointer** to `Circle`.",
    ...
    },
    {
      ...
      caption: { java: "`s.draw()` runs **`Circle.draw`** — chosen by the object's class, not by `s`'s type.",
...
const cppExplore = {
  ...
  code: `Shape* s = new Circle();\ns->draw();   // which draw?`,
  knobs: [
    { id: "virt", label: "Base method", options: [
      { value: "non",  label: "non-virtual" },
      { value: "virt", label: "virtual" },
    ], default: "non" },
  ],
  steps: (k) => k.virt === "virt"
    ? [ { line: 2, cells: [handle("s", "Shape*", "→ obj"), object("Circle", true), vtable("Circle::draw", true)],
          caption: { cpp: "`virtual` → **late binding**: `s->draw()` follows the object's vptr to the `Circle` vtable, so `Circle::draw` runs — like Java always does.",
                     intuition: "With `virtual`, the runtime type wins." } } ]
    : [ { line: 2, cells: [handle("s", "Shape*", "→ obj", true), object("Circle")],
          caption: { cpp: "Non-virtual → **early binding**: the call is resolved at compile time from the **static type** `Shape`, so `Shape::draw` runs.",
                     intuition: "C++ dispatches statically unless you write `virtual`; Java is always virtual." } } ],
};

export default quiz({ items: [
  { kind: "predict", scene: javaPredict },
  { kind: "explore", scene: cppExplore },
] });
