...
const WIN = { x: 24, y: 26, w: 262, h: diagramCardHeight(winSections) };
const AX = WIN.x + WIN.w + 14;   // annotation column

const note = (y, lines) => (
  <g>
    <line x1={WIN.x + WIN.w} y1={y} x2={AX + 4} y2={y}
      stroke="var(--mm-muted)" strokeWidth="1" opacity="0.5" />
    {lines.map((t, i) => (
      <text key={i} x={AX + 8} y={y + 4 + i * 14}
        style={{ fill: "var(--mm-muted)", fontSize: 10.5 }}>{t}</text>
    ))}
  </g>
);
...
export default function PkGui() {
  ...
  return (
    <DiagramSvg viewBox={`0 0 620 ${H}`} maxWidth={640}
      ...
      <DiagramCard x={WIN.x} y={WIN.y} w={WIN.w} title="Password Keeper" sections={winSections} sub={0} />
      {note(WIN.y + 40, ["txtTitle, txtPasscode : JTextField"])}
      {note(WIN.y + 62, ["btnAdd : JButton"])}
      {note(WIN.y + 116, ["gui_list : JList,", "backed by lstPasswords :", "DefaultListModel"])}
      {note(WIN.y + 178, ["btnDelete, btnExit : JButton"])}
      <text x={(WIN.x + 620) / 2} y={H - 8} textAnchor="middle"
