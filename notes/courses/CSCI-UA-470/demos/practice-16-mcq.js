...
export default mcq({
  questions: [{
    ...
    choices: [{
      text: "**Define** = declare a reference of that type (always legal); **create** = instantiate with `new` (legal only when nothing is left abstract)",
      correct: true
    }, {
    ...
    choices: [{
      text: "Only the concrete class — abstract classes and interfaces leave methods unimplemented, so `new` is illegal for them",
      correct: true
    }, {
    ...
    choices: [{
      text: "A class can **implement many** interfaces (only **extend one** class) — Java's controlled substitute for multiple inheritance — and interfaces now allow `default`/`static`/`private` method bodies",
      correct: true
    }, {
    ...
    choices: [{
      text: "Abstract class ↔ a class with **≥ 1** pure virtual (`= 0`); interface ↔ a class with **all** required methods pure virtual",
      correct: true
    }, {
    ...
    choices: [{
      text: "Multiple, and multi-level+multiple — to dodge the diamond problem; a class may `implement` many **interfaces** instead",
      correct: true
    }, {
