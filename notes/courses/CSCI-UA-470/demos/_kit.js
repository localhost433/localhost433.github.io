...
const ASM_MNEMONICS = new Set(["mov", "movabs", "lea", "push", "pop", "call", "ret", "leave", "jmp", "je", "jne", "jl", "jg", "jle", "jge", "cmp", "test", "add", "sub", "imul", "mul", "xor", "and", "or", "shl", "shr", "sar", "inc", "dec", "nop", "cdqe", "cqo", "syscall"]);
const ASM_REGS = new Set(["rax", "rbx", "rcx", "rdx", "rsi", "rdi", "rbp", "rsp", "rip", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15", "eax", "ebx", "ecx", "edx", "esi", "edi", "ebp", "esp", "r8d", "r9d", "al", "bl", "cl", "dl"]);
...
function highlightAsm(line) {
  return tokenize(line, ASM_RE, m => {
    ...
    if (m[4]) {
      ...
      return ASM_MNEMONICS.has(w) ? "mm-tok-kw" : ASM_REGS.has(w) ? "mm-tok-ty" : m[4].endsWith(":") ? "mm-tok-fn" : undefined;
    }
    return undefined;
  });
}
...
const highlight = (line, lang) => lang === "asm" ? highlightAsm(line) : lang === "bytecode" ? highlightBytecode(line) : highlightCode(line, lang);
...
const mkCell = region => (name, type, value, opts = {}) => ({
  ...
  addr: opts.addr,
  size: opts.size,
  // explicit byte size (overrides derivation)
  fields: opts.fields,
  // member list -> derived padded sizeof
  vptr: opts.vptr,
  // class has a virtual -> hidden vptr (+8)
  vptrOrigin: opts.vptrOrigin,
  // class the vptr belongs to (subobject colouring)
  header: opts.header,
  // object header bytes (e.g. Java)
...
export function obj(type, fields, base = {}) {
  ...
  return (id, vals = [], opts = {}) => make(id, type, "", {
    ...
    fields: fields.map((f, i) => ({
      ...f,
      value: Array.isArray(vals) ? vals[i] : vals ? vals[f.name] : undefined
    }))
  });
}
...
export function derived(layers, base = {}) {
  ...
  return (id, vals = {}, opts = {}) => make(id, type, "", {
    ...base,
    ...opts,
    vptr: vlayer ? vlayer.vptr : undefined,
    // true = slot only; string = points at that vtable id
...
export function part(label, type, fields, id, region = "stack") {
  ...
  return (hl, destroyed) => make(label, type, destroyed ? fields.map(f => f.value).filter(v => v != null).join(", ") : "", {
    id,
    hl,
    destroyed,
    fields
...
export function ladder(lines) {
  return n => lines.slice(0, n).map(result => ({
    expr: "",
    result
  }));
}
...
function cellBytes(cell) {
  ...
  if (cell.fields || cell.vptr || cell.header) return sizeLayout(cell.fields || [], {
    vptr: cell.vptr,
    header: cell.header
  }).total;
  const t = cell.type;
...
export function sizeLayout(fields, opts = {}) {
  ...
  if (opts.vptr) {
    // hidden vtable pointer; a string value means it points at that cell id
    blocks.push({
      kind: "vptr",
      size: 8,
      offset,
      to: typeof opts.vptr === "string" ? opts.vptr : undefined,
      link: "ptr",
      sub: subOf(opts.vptrOrigin)
    });
    offset += 8;
  }
  fields.forEach(f => {
...
function LayoutBar({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-layout__bar" + (value ? " mm-layout__bar--value" : "")
  }, blocks.map((b, i) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      ref: !value && regRef ? el => {
        regRef("b:" + cellId + ":" + i, el);
        if (b.anchor) regRef(b.anchor, el);
      } : undefined,
      className: cls.join(" "),
      style: {
        flexGrow: b.size
      ...
      className: "mm-layout__val",
      title: b.value != null ? String(b.value) : undefined
    }, b.value != null ? String(b.value) : "") : /*#__PURE__*/React.createElement("span", {
      className: "mm-layout__lbl"
    }, b.name ? b.name : b.kind), value ? null : /*#__PURE__*/React.createElement("span", {
      className: "mm-layout__sz"
    }, b.size));
  }));
}
export function SizeLayout({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-layout__title"
  }, title) : null, /*#__PURE__*/React.createElement(LayoutBar, {
    fields: fields,
    vptr: vptr,
    header: hdr
  }), /*#__PURE__*/React.createElement("div", {
    className: "mm-layout__foot"
  }, /*#__PURE__*/React.createElement("span", null, lang === "java" ? "object size" : "sizeof", " = ", /*#__PURE__*/React.createElement("b", null, total, " bytes")), /*#__PURE__*/React.createElement("span", {
    className: "mm-layout__align"
  }, "aligned to ", align)));
}
...
export function SizeCompare({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-sizes"
  }, items.map((it, i) => /*#__PURE__*/React.createElement(SizeLayout, _extends({
    key: i,
    lang: lang
  }, it))));
}
const SEGMENTS = [{
  key: "stack",
  label: "Stack",
  hint: "locals & args · auto, LIFO"
}, {
  key: "heap",
  label: "Heap",
  hint: "new / delete · manual lifetime"
}, {
  key: "global",
  label: "Global / Static",
  hint: "globals, statics, literals"
}, {
  key: "code",
  label: "Code",
...
function Cell({
  ...
  const cls = ["mm-cell", isPtr ? "mm-cell--ptr" : "", cell.kind === "ref" ? "mm-cell--ref" : "", cell.highlight ? "mm-cell--hl" : "", cell.reclaimed ? "mm-cell--reclaimed" : "", cell.destroyed ? "mm-cell--destroyed" : ""].filter(Boolean).join(" ");
  const bytes = cellBytes(cell);
  const w = cellWidthRem(bytes);
  const style = w != null ? {
    width: w.toFixed(2) + "rem"
  } : undefined;
  ...
  const stripFields = cell.fields || (bytes != null ? [{
    name: cell.type || "",
    size: bytes,
    value: cell.value
  }] : null);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-cell__top"
  }, cell.name ? /*#__PURE__*/React.createElement("span", {
    className: "mm-cell__name"
  }, cell.name) : /*#__PURE__*/React.createElement("span", {
    className: "mm-cell__name mm-cell__name--anon"
  }, "(unnamed)")), cell.type ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cell__typerow"
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-cell__type"
  }, cell.type)) : null, cell.reclaimed || cell.destroyed ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cell__value mm-cell__value--reclaimed"
  }, cell.value) : cell.value && !showStrip ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cell__value"
  }, cell.value) : null, cell.addr ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cell__meta"
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-cell__addr"
  }, cell.addr)) : null, showStrip ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cell__table"
  }, /*#__PURE__*/React.createElement(LayoutBar, {
    fields: stripFields,
    vptr: cell.vptr,
    header: cell.header,
    vptrOrigin: cell.vptrOrigin,
    variant: "value"
  }), /*#__PURE__*/React.createElement(LayoutBar, {
    fields: stripFields,
    vptr: cell.vptr,
...
export function MemoryModel({
  ...
  const sig = JSON.stringify(cells.map(c => [c.id, c.region, c.pointsTo, c.link, c.reclaimed, c.destroyed, c.value, c.vptr, c.header, c.fields && c.fields.map(f => [f.name, f.size, f.type, f.to, f.link])]));
    ...
    const compute = () => {
      ...
      const add = (id, srcEl, tgtId, link) => {
        ...
        reqs.push({
          ...
          tgtId,
          link,
          srcRect: rel(srcEl.getBoundingClientRect()),
          tgtRect: rel(t.getBoundingClientRect())
        });
      };
      cells.forEach(c => {
      ...
      const byTgt = {};
      reqs.forEach(r => {
        (byTgt[r.tgtId] = byTgt[r.tgtId] || []).push(r);
      });
      Object.keys(byTgt).forEach(tgtId => {
        ...
        group.forEach((r, idx) => {
          ...
          next.push({
            id: r.id,
            link: reclaimedIds.has(r.tgtId) ? "dangling" : r.link || "ptr",
            dot: g.dot,
            d: g.d
          });
        });
  ...
  cells.forEach(c => {
    ...
    (c.fields || []).forEach(f => {
      if (f.origin && !subOrigins.includes(f.origin)) subOrigins.push(f.origin);
    });
  });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    d: "M3,2.5 L7.5,5 L3,7.5",
    className: "mm-head mm-head--dangling"
  }))), lines.map(l => {
    ...
    className: "mm-axis__cap"
  }, "high"), /*#__PURE__*/React.createElement("span", {
    className: "mm-axis__line"
  }), /*#__PURE__*/React.createElement("span", {
    className: "mm-axis__cap"
  }, "low")) : null, /*#__PURE__*/React.createElement("div", {
    className: "mm-segs"
  }, segs.map(seg => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, {
      ...
      className: "mm-seg mm-seg--" + seg.key + (list.length ? "" : " mm-seg--empty") + (activeSet.has(seg.key) ? " mm-seg--active" : "")
    }, /*#__PURE__*/React.createElement("div", {
      className: "mm-seg__head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "mm-seg__label"
    }, seg.label), /*#__PURE__*/React.createElement("span", {
      className: "mm-seg__hint"
    }, seg.hint)), list.length ? /*#__PURE__*/React.createElement("div", {
      className: "mm-seg__cells"
    }, list.map(c => /*#__PURE__*/React.createElement(Cell, {
      key: c.id,
      cell: c,
      attachRef: el => {
      ...
      regRef: regRef,
      onEnter: e => showTip(c, e),
      onLeave: hideTip
    }))) : null), seg.key === "stack" && heapVisible ? /*#__PURE__*/React.createElement("div", {
      className: "mm-gap"
    }, hasNull ? /*#__PURE__*/React.createElement("span", {
      className: "mm-null",
      ref: el => regRef(NULL_ID, el)
    }, "\u2300 nullptr") : null, /*#__PURE__*/React.createElement("span", {
      className: "mm-gap__txt"
    }, "free / unmapped \xB7 stack & heap grow toward each other")) : null);
  })), tip ? /*#__PURE__*/React.createElement("div", {
    className: "mm-tip mm-tip--" + (tip.above ? "above" : "below"),
    style: {
      left: tip.left + "px",
      top: tip.top + "px"
    },
    role: "tooltip"
  }, tip.lines.map(([k, t], i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "mm-tip__line mm-tip__" + k
  }, t))) : null), showLegend ? /*#__PURE__*/React.createElement("div", {
    className: "mm-legend"
  }, subOrigins.map((o, i) => /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item",
    key: "sub" + i
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--sub" + i
  }), " ", o)), hasPtr ? /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item"
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--ptr"
  }), " pointer") : null, hasRef ? /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item"
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--ref"
  }), " reference (alias)") : null, hasReclaimed ? /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item"
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--reclaimed"
  }), " reclaimed memory") : null, hasDangling ? /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item"
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--dangling"
  }), " dangling pointer") : null, hasDestroyed ? /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item"
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--destroyed"
...
export function ObjectLayout({
  ...
  blocks.forEach(b => {
    ...
    if (last && last.sub === key && key != null) last.size += b.size;else groups.push({
      sub: key,
      size: b.size,
      label: key == null ? "" : origins[key]
    });
  });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ol__title"
  }, title) : null, ptrMarks.length ? /*#__PURE__*/React.createElement("div", {
    className: "ol__ptrs",
    style: {
      height: 2 + 1.8 * maxPtrLvl + "rem"
    }
  }, ptrMarks.map((p, i) => /*#__PURE__*/React.createElement("div", {
    className: "ol__ptr ol__ptr--" + (p.link || "ptr") + " ol__ptr--" + p.side,
    key: "p" + i,
    style: {
      left: p.pct + "%",
      "--lvl": p.lvl
    ...
    className: "ol__ptr-box"
  }, /*#__PURE__*/React.createElement("span", {
    className: "ol__ptr-name"
  }, p.name), /*#__PURE__*/React.createElement("span", {
    className: "ol__ptr-off"
  }, "@ +", p.at)), /*#__PURE__*/React.createElement("span", {
    className: "ol__ptr-stem"
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      className: cls.join(" "),
      title: (b.name || b.kind) + " @ +" + b.offset + " · " + b.size + "B"
    }, /*#__PURE__*/React.createElement("span", {
      className: "ol__cell-lab"
    }, b.name ? b.name : b.kind), b.value != null ? /*#__PURE__*/React.createElement("span", {
      className: "ol__cell-val"
    }, String(b.value)) : null);
  })), /*#__PURE__*/React.createElement("div", {
    className: "ol__ruler",
    style: {
    ...
    key: i,
    className: "ol__tick"
  }, /*#__PURE__*/React.createElement("span", null, "+", b.offset))), /*#__PURE__*/React.createElement("span", {
    className: "ol__tick-end"
  }, "+", total)), /*#__PURE__*/React.createElement("div", {
    className: "ol__brk",
    style: {
      gridTemplateColumns: brkCols
    }
  }, groups.map((g, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "ol__grp" + (g.sub == null ? " ol__grp--gap" : " mm-layout__block--sub" + g.sub)
  }, g.label ? /*#__PURE__*/React.createElement("span", null, g.label) : null))), note ? /*#__PURE__*/React.createElement("div", {
    className: "ol__note"
  }, renderCaption(note)) : null, origins.length ? /*#__PURE__*/React.createElement("div", {
    className: "mm-legend"
  }, origins.map((o, i) => /*#__PURE__*/React.createElement("span", {
    className: "mm-legend__item",
    key: i
  }, /*#__PURE__*/React.createElement("i", {
    className: "mm-swatch mm-swatch--sub" + i
...
function validateAsmMap(asm, asmMap) {
  ...
  for (const src of Object.keys(asmMap)) {
    for (const a of asmMap[src]) {
      if (a < 1 || a > N) console.warn(`[asmMap] src ${src} -> asm line ${a} out of range (1..${N})`);else if (isElision(lines[a - 1] || "")) console.warn(`[asmMap] src ${src} -> asm line ${a} is an elided row`);
    }
  }
}
...
export function CodeAsmPane({
  ...
  if (hover && hover.side === "src") {
    ...
    hlAsm = activeAsmLine != null ? Array.isArray(activeAsmLine) ? activeAsmLine : [activeAsmLine] : (Array.isArray(activeSrcLine) ? activeSrcLine : [activeSrcLine]).flatMap(l => asmMap[l] || []);
  }
  const column = (head, props) => /*#__PURE__*/React.createElement("div", {
    className: "mm-codeasm__col"
  }, /*#__PURE__*/React.createElement("div", {
    className: "mm-codeasm__head"
  }, head), /*#__PURE__*/React.createElement(CodeBlock, props));
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-codeasm"
  }, column(SRC_LABEL[lang] || "source", {
    code,
    activeLine: hlSrc,
    lang,
    onHoverLine: n => setHover(n == null ? null : {
      side: "src",
      line: n
    }),
    onPickLine: onPickSrc,
    pickable: pickableSrc
    ...
    onPickLine: onPickAsm,
    pickable: pickableAsm
  }));
}
function Section({
  title,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-sec__chev" + (open ? " mm-sec__chev--open" : ""),
    "aria-hidden": "true"
  }, "\u25B8"), /*#__PURE__*/React.createElement("span", null, title)), open ? /*#__PURE__*/React.createElement("div", {
    className: "mm-sec__body"
  }, children) : null);
}
...
export function PredictGate({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-predict"
  }, /*#__PURE__*/React.createElement("p", {
    className: "mm-predict__ask"
  }, renderCaption(predict.ask)), /*#__PURE__*/React.createElement("div", {
    className: "mm-predict__choices"
  }, predict.choices.map((c, k) => /*#__PURE__*/React.createElement("button", {
    key: k,
    type: "button",
    className: "mm-predict__choice",
    onClick: () => onAnswer(k)
  }, renderCaption(c.label)))), /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: "mm-predict__skip",
    onClick: () => onAnswer(-1)
  }, "Just show me"));
}
export function Verdict({
  ...
  return /*#__PURE__*/React.createElement("p", {
    ...
    className: "mm-verdict__mark mm-verdict__mark--" + (chosen.correct ? "ok" : "no")
  }, chosen.correct ? "✓ correct" : "✗ not quite") : null, predict.why ? /*#__PURE__*/React.createElement("span", {
    className: "mm-cap-txt"
  }, renderCaption(predict.why)) : null);
}
...
export function WhyDot({
  ...
  return /*#__PURE__*/React.createElement("g", {
    className: "mm-why__dot" + (active ? " mm-why__dot--on" : ""),
    role: "button",
    tabIndex: 0,
    "aria-pressed": !!active,
...
export function WhyNotes({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-why__note"
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-why__badge"
  }, open), /*#__PURE__*/React.createElement("span", {
    className: "mm-cap-txt"
  }, /*#__PURE__*/React.createElement("strong", null, active.title, ". "), renderCaption(active.body))) : /*#__PURE__*/React.createElement("p", {
    className: "mm-why__hint"
  }, "Tap a numbered marker to see how each piece works."));
}
...
export function BuilderControls({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "bex-controls"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-status",
    role: "status",
    "aria-live": "polite"
  }, status), /*#__PURE__*/React.createElement("div", {
    className: "bex-controls__btns"
  }, /*#__PURE__*/React.createElement(Button, {
    onClick: onCheck,
    disabled: checkDisabled || revealed
...
export function gradedChipState({
  revealed,
  checked,
  filled,
  ok
...
export function MemoryScene({
  ...
  const nav = React.useMemo(() => {
    ...
    return {
      src,
      asmL,
      srcSet: new Set(Object.keys(src).map(Number)),
      asmSet: new Set(Object.keys(asmL).map(Number))
    };
  }, [resolved, asmMap]);
  const pickSrc = L => {
    const s = nav.src[L];
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, title) : null, knobs && knobs.length ? /*#__PURE__*/React.createElement(KnobBar, {
    knobs: knobs,
    value: knob,
    onChange: setKnobVal
    ...
    className: "mm-predict-wrap"
  }, prevStep ? /*#__PURE__*/React.createElement("div", {
    className: "mm-predict-ctx"
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-predict-ctx__tag"
  }, "state now \u2014 predict what changes"), prevStep.layout ? /*#__PURE__*/React.createElement(ObjectLayout, prevStep.layout) : /*#__PURE__*/React.createElement(MemoryModel, {
    ...
    predict: pred,
    onAnswer: answer
  })) : step.layout ? /*#__PURE__*/React.createElement(ObjectLayout, step.layout) : /*#__PURE__*/React.createElement(MemoryModel, {
    cells: step.cells,
    segments: segments,
    active: step.active,
    axis: axis
  ...
  }, step.asm || asm ? /*#__PURE__*/React.createElement(CodeAsmPane, {
    code: step.code || code,
    lang: lang,
    asm: step.asm || asm,
    ...
    onPickSrc: pickSrc,
    pickableSrc: nav.srcSet,
    onPickAsm: pickAsm,
    pickableAsm: nav.asmSet
    ...
    onPickLine: pickSrc,
    pickable: nav.srcSet
  })), answered && step.outputs ? /*#__PURE__*/React.createElement(Section, {
    title: outLabel || "Output (cout)",
    open: open.out,
    onToggle: () => toggle("out")
  }, /*#__PURE__*/React.createElement("div", {
    className: "mm-console"
  }, step.outputs.map((o, k) => /*#__PURE__*/React.createElement("div", {
    className: "mm-console__row",
    key: k
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-console__expr"
  }, o.expr), /*#__PURE__*/React.createElement("span", {
    className: "mm-console__arrow"
  }, "prints"), /*#__PURE__*/React.createElement("span", {
    className: "mm-console__out"
  }, o.result), o.note ? /*#__PURE__*/React.createElement("span", {
    className: "mm-console__note"
  }, o.note) : null)))) : null, answered && pred ? /*#__PURE__*/React.createElement(Verdict, {
    predict: pred,
    pick: picks[i2]
  }) : null, answered ? step.caption && typeof step.caption === "object" ? /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__caption mm-scene__caption--struct"
  }, CAPTION_ROWS.map(([key, label, mod]) => step.caption[key] != null ? /*#__PURE__*/React.createElement("p", {
    className: "mm-cap-row",
    key: key
  }, /*#__PURE__*/React.createElement("span", {
    className: "mm-cap-tag mm-cap-tag--" + mod
  }, label), /*#__PURE__*/React.createElement("span", {
    className: "mm-cap-txt"
  }, renderCaption(step.caption[key]))) : null)) : step.caption != null ? /*#__PURE__*/React.createElement("p", {
    className: "mm-scene__caption"
  }, renderCaption(step.caption)) : null : null, /*#__PURE__*/React.createElement("div", {
    ...
    role: "tablist"
  }, resolved.map((_, n) => /*#__PURE__*/React.createElement("button", {
    key: n,
    type: "button",
    "aria-label": "Step " + (n + 1),
    "aria-selected": n === i2,
    className: "mm-dot" + (n === i2 ? " mm-dot--on" : ""),
    onClick: () => setI(n)
  }))), /*#__PURE__*/React.createElement(Button, {
    variant: "outline",
    size: "sm",
    disabled: i2 === resolved.length - 1,
    onClick: () => setI(i2 + 1)
  }, "Next"), /*#__PURE__*/React.createElement("span", {
    className: "mm-scene__step"
  }, i2 + 1, " / ", resolved.length)));
}
...
function DualColumn({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    activeLine: step.line,
    lang: side.lang || "cpp"
  }), step.caption && typeof step.caption === "object" ? /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__caption mm-scene__caption--struct"
  }, CAPTION_ROWS.map(([key, label, mod]) => step.caption[key] != null ? /*#__PURE__*/React.createElement("p", {
    className: "mm-cap-row",
    key: key
    ...
    className: "mm-cap-txt"
  }, renderCaption(step.caption[key]))) : null)) : /*#__PURE__*/React.createElement("p", {
    className: "mm-scene__caption"
...
export function MemoryDualScene({
  title,
  left,
  right
}) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-scene mm-dual-wrap"
  }, title ? /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__title",
    "data-artifact-title": true
  }, title) : null, /*#__PURE__*/React.createElement("div", {
    className: "mm-dual"
  }, /*#__PURE__*/React.createElement(DualColumn, {
    side: left,
    step: at(left)
  }), /*#__PURE__*/React.createElement(DualColumn, {
    side: right,
    ...
    length: total
  }, (_, n) => /*#__PURE__*/React.createElement("button", {
    key: n,
    type: "button",
    "aria-label": "Step " + (n + 1),
    "aria-selected": n === i,
...
export function DiagramBox({
  ...
  }, label), note ? /*#__PURE__*/React.createElement("text", {
    x: cx,
    y: cy + 11,
    textAnchor: "middle",
    dominantBaseline: "central",
...
export function Pipeline({
  ...
  return /*#__PURE__*/React.createElement(DiagramSvg, {
    ...
    maxWidth: maxWidth,
    ariaLabel: ariaLabel
  }, steps.map((s, i) => /*#__PURE__*/React.createElement("g", {
    key: "z" + i
    ...
    rx: 9,
    style: {
      fill: `var(${s.accent ? "--seg-global-bg" : "--mm-reclaimed-bg"})`,
      opacity: 0.5
    }
  }), /*#__PURE__*/React.createElement("rect", {
    x: 6.75,
  ...
  }, s.zone))), boxed.slice(1).map((bi, k) => /*#__PURE__*/React.createElement(DiagramEdge, {
    key: "e" + bi,
    from: {
      x: colX,
      y: boxBot(boxed[k])
  ...
  })), steps.map((s, i) => s.via ? /*#__PURE__*/React.createElement("text", {
    key: "v" + i,
    x: colX + 14,
    y: s.label ? bandTop(i) + 17 : bandMid(i) + 4,
    textAnchor: "start",
    style: {
      fill: "var(--mm-muted)",
      fontSize: 11,
  ...
  }, s.via) : null), steps.map((s, i) => s.feed ? /*#__PURE__*/React.createElement("g", {
    key: "f" + i
    ...
    note: s.feed.note,
    sub: s.feed.sub
  })) : null), steps.map((s, i) => s.label ? /*#__PURE__*/React.createElement(DiagramBox, {
    key: "b" + i,
    cx: colX,
    cy: bandMid(i),
    w: BW,
...
const sectionHeight = s => DCARD.SECT_PAD * 2 + Math.max(1, (s.rows || []).length) * DCARD.ROW_H;
...
export function DiagramCard({
  ...
  const h = diagramCardHeight(sections, {
    title: title != null
  });
  ...
  }), title != null ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("text", {
    x: x + w / 2,
    y: y + titleH / 2 + 0.5,
    textAnchor: "middle",
    dominantBaseline: "central",
  ...
  })) : null, sections.map((s, si) => {
    const top = cy;
    cy += sectionHeight(s);
    return /*#__PURE__*/React.createElement("g", {
      key: si
    }, si > 0 ? /*#__PURE__*/React.createElement("line", {
      x1: x,
      y1: top,
      x2: x + w,
      y2: top,
...
export function InheritFork({
  ...
  const s = {
    stroke: impl ? "var(--mm-ref)" : "var(--mm-ptr)",
    strokeWidth: 1.5,
  ...
  return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
    ...
    y2: busY,
    style: s
  }), childCxs.map((cx, i) => /*#__PURE__*/React.createElement("line", {
    key: i,
    x1: cx,
    y1: busY,
    x2: cx,
...
export function treeLayout({
  ...
  const ph = diagramCardHeight(parent.sections, {
    title: true
  });
  ...
  const kids = children.map((spec, i) => {
    ...
    return {
      ...
      cx: x + cardW / 2,
      top: childTop,
      h: diagramCardHeight(spec.sections, {
        title: true
      })
    };
  });
...
export function ClassTree({
  ...
  const card = (c, key) => /*#__PURE__*/React.createElement(DiagramCard, {
    key: key,
    x: c.x,
    y: c.y,
    w: c.w,
    title: c.spec.title,
    sections: c.spec.sections,
    neutral: true,
    abstract: c.spec.abstract,
    dashed: c.spec.dashed
  });
  return /*#__PURE__*/React.createElement("g", null, card(layout.parent, "p"), layout.children.map((k, i) => card(k, i)), /*#__PURE__*/React.createElement(InheritFork, {
    parentCx: layout.parent.cx,
    parentBottomY: layout.parent.bottom,
    childCxs: layout.children.map(k => k.cx),
    childTopY: layout.childTop,
    busY: layout.busY,
    relation: relation
  }));
}
...
export function patternTree({
  ...
  gapY = 40,
  place = "left",
  // where the context card sits relative to the abstract parent
  parent,
  children = [],
  relation = "extends",
  cardW = 150,
  ...
  const ctxH = context ? diagramCardHeight(context.sections, {
    title: true
  }) : 0;
  const parentH = diagramCardHeight(parent.sections, {
    title: true
  });
...
function PatternHalf({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-pat__half"
  }, tag ? /*#__PURE__*/React.createElement("span", {
    className: "mm-pat__tag mm-cap-tag mm-cap-tag--" + (tagKind || "cpp")
  }, tag) : null, isCode ? /*#__PURE__*/React.createElement(CodeBlock, {
    code: half.code,
    lang: half.lang || "java"
  }) : /*#__PURE__*/React.createElement(DiagramSvg, {
    viewBox: half.viewBox,
    ariaLabel: half.ariaLabel,
    maxWidth: half.maxWidth || maxWidth
  }, half.node, cross ? /*#__PURE__*/React.createElement(CrossOut, {
    x: 0,
    y: 0,
    w: half.width,
    h: half.height
  }) : null), half.note ? /*#__PURE__*/React.createElement("p", {
    className: "mm-pat__note"
  }, renderCaption(half.note)) : null);
}
export function PatternFigure({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-scene mm-pat"
  }, title ? /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__title",
    "data-artifact-title": true
  }, title) : null, intent ? /*#__PURE__*/React.createElement("p", {
    className: "mm-pat__intent"
  }, intent) : null, /*#__PURE__*/React.createElement(PatternHalf, {
    half: bad,
    tag: badTag,
    tagKind: "asm",
    cross: bad && bad.code == null,
    maxWidth: maxWidth
  }), /*#__PURE__*/React.createElement(PatternHalf, {
    half: good,
    tag: goodTag,
    tagKind: "int",
    maxWidth: maxWidth
    ...
    className: "mm-pat__tag mm-cap-tag mm-cap-tag--java"
  }, client.label || "client code"), /*#__PURE__*/React.createElement(CodeBlock, {
    code: client.code,
    lang: client.lang || "java"
  }), client.note ? /*#__PURE__*/React.createElement("p", {
    className: "mm-pat__note"
  }, renderCaption(client.note)) : null) : null, caption ? /*#__PURE__*/React.createElement(CompareCaption, {
    cols: caption.cols,
    punch: caption.punch
  }) : null);
}
export function patternFigure(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(PatternFigure, cfg);
  };
}
...
export function svgCodeSize(lines = [], title) {
  ...
  return {
    w: Math.round(2 * SVGC.PAD + widest * SVGC.CH),
...
export function SvgCode({
  ...
  }, title) : null, lines.map((l, i) => /*#__PURE__*/React.createElement("text", {
    key: i,
    x: x + SVGC.PAD,
...
export function UseCaseOval({
  ...
  const c = sub != null ? diagramPalette(sub) : {
    bg: "--mm-panel-bg",
    bd: "--mm-cell-bd",
    fg: "--mm-cell-fg"
  };
...
const UML_LINK = {
  ...
  aggregate: {
    ...
    dashed: false,
    marker: null,
    startMarker: "dia-diamond-hollow",
    label: null
  },
  compose: {
    color: "--mm-muted",
    dashed: false,
    marker: null,
    startMarker: "dia-diamond-filled",
    label: null
  }
};
...
export function UmlLink({
  ...
  if (orth && !axis) {
    ...
    shape = /*#__PURE__*/React.createElement("line", {
      x1: from.x,
      y1: from.y,
      x2: to.x,
      y2: to.y,
  ...
  return /*#__PURE__*/React.createElement("g", null, shape, lab ? /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("rect", {
    x: lx + labelDx - labW / 2,
    y: ly + labelDy - 10.5,
    width: labW,
    height: 14,
...
const MONO_CH = 7.2,
  UI_CH = 6.2; // approx px per character at the sizes we render
...
export function UseCaseDiagram({
  ...
  const cs = cases.map((c, i) => {
    ...
    return {
      ...
      sub: o.sub,
      dashed: o.dashed,
      rx: o.rx != null ? o.rx : ovalRx(o.label),
      ry: o.ry || ry,
      cx: o.x,
      cy: o.y
    };
  ...
  const bx = minX - padX,
    bw = maxX - minX + 2 * padX;
  const bTop = minY - padTop,
    bh = maxY - minY + padTop + padBot;
  ...
  const tieGroups = {};
  acts.forEach(a => {
    if (a.y == null) (tieGroups[Math.round(a.midY)] = tieGroups[Math.round(a.midY)] || []).push(a);
  });
  Object.values(tieGroups).forEach(g => {
    if (g.length < 2) return;
    const c = g.reduce((s, a) => s + a.midY, 0) / g.length;
    g.forEach((a, k) => {
      a.midY = c + (k - (g.length - 1) / 2) * rowGap;
    });
  });
  ...
  const shareCount = {};
  acts.forEach(a => a.linked.forEach(c => {
    shareCount[c.id] = (shareCount[c.id] || 0) + 1;
  }));
  ...
  const rels = relations.map((r, i) => {
    ...
    } else if (Math.abs(dy) < A.ry * 0.5) {
      // same row → horizontal
      const right = dx > 0;
      from = {
        x: A.cx + (right ? A.rx : -A.rx),
        y: A.cy
      };
      to = {
        x: B.cx + (right ? -B.rx : B.rx),
        y: A.cy
      };
    } else {
      // offset → straight diagonal to the rims
  ...
  if (showRoles && acts.length === 1 && cs.length === 1) {
    ...
    roles.forEach(r => {
      r.y = rY;
      b.text(r.x, rY, estHalf(r.t, UI_CH));
    });
  }
  let capY = null;
  if (caption) {
    capY = b.y1 + 24;
    b.text(boundCX, capY, estHalf(caption.text || caption, UI_CH));
  }
  const aria = `Use-case diagram${system ? " for " + system : ""}: ` + acts.map(a => joinLabel(a.label)).join(", ") + (acts.length ? " — " : "") + cs.map(c => joinLabel(c.label)).join(", ") + ".";
  ...
  const pickable = (id, kind, name, box, isOval, glyph) => {
    ...
    }, ring ? isOval ? /*#__PURE__*/React.createElement("ellipse", {
      cx: box.x + box.w / 2,
      cy: box.y + box.h / 2,
      rx: box.w / 2 + 5,
      ry: box.h / 2 + 5,
  ...
  return /*#__PURE__*/React.createElement(DiagramSvg, {
    viewBox: b.viewBox(),
    maxWidth: maxWidth || Math.min(720, Math.round(b.width())),
    ariaLabel: aria
  }, system ? /*#__PURE__*/React.createElement(SystemBoundary, {
    x: bx,
    y: bTop,
    w: bw,
    h: bh,
    label: system
  }) : null, branchSets.map(bs => {
    ...
    return /*#__PURE__*/React.createElement("g", {
      key: bs.actor
    }, onRow ? /*#__PURE__*/React.createElement(UmlLink, {
      kind: "assoc",
      from: {
        x: bs.armX,
        y: bs.midY
      ...
      y2: bs.midY,
      style: s
    }), !bs.single ? /*#__PURE__*/React.createElement("line", {
      x1: bs.spineX,
      y1: Math.min(...ys),
      x2: bs.spineX,
      y2: Math.max(...ys),
      style: s
    }) : null, others.map(br => /*#__PURE__*/React.createElement(UmlLink, {
      key: br.cid,
      kind: "assoc",
      from: {
        x: bs.spineX,
  ...
  }), rels.filter(r => vRel(r.id)).map(r => /*#__PURE__*/React.createElement(UmlLink, {
    key: r.id,
    kind: r.kind,
    from: r.from,
    to: r.to,
    ...
    const glyph = /*#__PURE__*/React.createElement(Actor, {
      key: a.id,
      x: a.ax,
      y: a.midY - Actor.ARM,
      label: a.label,
    ...
    return pickable(a.id, "actor", joinLabel(a.label), {
      x: a.ax - 20,
      y: y0 - 3,
      w: 40,
      h: 80
    ...
    const glyph = /*#__PURE__*/React.createElement(UseCaseOval, {
      key: c.id,
      cx: c.cx,
      cy: c.cy,
      rx: c.rx,
    ...
    if (!onPick) return glyph;
    return pickable(c.id, "case", joinLabel(c.label), {
      x: c.cx - c.rx,
      y: c.cy - c.ry,
      w: 2 * c.rx,
      h: 2 * c.ry
    }, true, glyph);
  }), roles ? roles.map((r, i) => /*#__PURE__*/React.createElement("text", {
    key: "rl" + i,
    x: r.x,
    y: r.y,
    textAnchor: "middle",
  ...
  }, r.t)) : null, caption ? /*#__PURE__*/React.createElement(FigureCaption, {
    x: boundCX,
    y: capY,
...
export function UseCaseRelation({
  focal,
  satellites = [],
  kind = "generalize",
  caption,
  ...
  const f = typeof focal === "string" ? {
    label: focal
  } : {
    ...focal
  };
  const sats = satellites.map(s => typeof s === "string" ? {
    label: s
  } : {
    ...s
  });
  ...
  f.cy = 0;
  const gapX = 132,
    sGap = 34; // stack the satellites to the right
  const totalH = sats.length * 2 * ry + (sats.length - 1) * sGap;
  let y = -totalH / 2 + ry;
  sats.forEach(s => {
    s.cx = 2 * rx + gapX;
    s.cy = sats.length === 1 ? 0 : y;
    s.rx = rx;
    s.ry = ry;
    y += 2 * ry + sGap;
  });
  ...
  if (caption) {
    capY = b.y1 + 26;
    b.text((b.x0 + b.x1) / 2, capY, estHalf(caption.text || caption, UI_CH));
  }
  const aria = `${kind} relationship: ${sats.map(s => Array.isArray(s.label) ? s.label.join(" ") : s.label).join(", ")} ` + `${kind === "generalize" ? "generalize to" : kind === "include" ? "each include" : "each extend"} ` + `${Array.isArray(f.label) ? f.label.join(" ") : f.label}.`;
  return /*#__PURE__*/React.createElement(DiagramSvg, {
    viewBox: b.viewBox(),
    maxWidth: maxWidth || Math.min(560, Math.round(b.width())),
    ariaLabel: aria
  }, sats.map((s, i) => /*#__PURE__*/React.createElement(UmlLink, {
    key: i,
    kind: kind,
    from: ovalEdge(s.cx, s.cy, s.rx, s.ry, f.cx, f.cy),
    to: ovalEdge(f.cx, f.cy, f.rx, f.ry, s.cx, s.cy),
    labelDy: -7
  })), /*#__PURE__*/React.createElement(UseCaseOval, {
    cx: f.cx,
    cy: f.cy,
    rx: f.rx,
    ry: f.ry,
    label: f.label,
    sub: f.sub
  }), sats.map((s, i) => /*#__PURE__*/React.createElement(UseCaseOval, {
    key: i,
    cx: s.cx,
    cy: s.cy,
    rx: s.rx,
    ...
    sub: s.sub,
    dashed: s.dashed
  })), caption ? /*#__PURE__*/React.createElement(FigureCaption, {
    x: (b.x0 + b.x1) / 2,
    y: capY,
    text: caption.text || caption,
    color: capColor
...
export function UseCaseWalkthrough({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-scene__title",
    "data-artifact-title": true
  }, title) : null, /*#__PURE__*/React.createElement(UseCaseDiagram, _extends({}, spec, {
    caption: null,
    visible: step.show,
    maxWidth: maxWidth
  })), step.caption ? /*#__PURE__*/React.createElement("p", {
    className: "mm-scene__caption"
  }, renderCaption(step.caption)) : null, /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__nav"
  }, /*#__PURE__*/React.createElement(Button, {
    variant: "ghost",
    size: "sm",
...
export const librarySystem = {
  ...
  associations: [{
    ...
    actor: "librarian",
    cases: ["add", "remove", "update", "organize", "list"]
  }],
  relations: [{
    id: "e-borrow",
    from: "borrow",
...
export const librarySteps = [{
  ...
  show: {
    actors: BOTH,
    cases: ["search", "add", "remove", "update", "organize"],
    relations: []
  }
}, {
...
export const converterUseCase = {
  ...
  cases: [{
    ...
    id: "cminch",
    label: "Convert Cm → Inch",
    x: 430,
    y: 220
  }, {
    id: "convert",
    label: "Convert(amount, targetUnit)",
    x: 760,
    y: 150,
    sub: 1
  }],
...
export const converterUseCaseSteps = [{
  ...
  show: {
    actors: ["user"],
    cases: ["kglb", "cminch", "convert"],
    relations: ["g-kglb", "g-cminch"]
  }
}];
...
export const soundRecorder = {
  ...
  caption: {
    text: "the three refinements «extend» the View Recordings hub — each an optional add-on",
    color: "--mm-muted"
  }
};
...
export function useCaseRelation(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(UseCaseRelation, cfg);
  };
}
export function useCaseWalkthrough(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(UseCaseWalkthrough, cfg);
  };
}
...
export function SequenceDiagram({
  ...
  fragments.forEach(f => {
    ...
    (f.dividers || []).forEach(d => {
      if (rowPad[d.at] != null) rowPad[d.at] += SEQ.BAND;
    });
  });
  const yOf = [];
  {
  ...
  const b = makeBounds();
  ps.forEach(p => {
    if (p.kind === "actor") b.actor(p.cx, headTop(p), p.label);else b.box(p.cx - halfW(p), headTop(p), 2 * halfW(p), SEQ.BOX_H);
  ...
  messages.forEach((m, i) => {
    ...
    const midX = (A.cx + B.cx) / 2,
      halfLab = estHalf(m.label || "", MONO_CH);
    b.text(midX, y - 6, Math.max(halfLab, 20));
  });
  fragments.forEach(f => b.box(minX - 34, msgY(f.from) - SEQ.BAND - 8, maxX - minX + 68, msgY(f.to) - msgY(f.from) + SEQ.BAND + 30));
  ...
  if (caption) {
    capY = b.y1 + 24;
    b.text((minX + maxX) / 2, capY, estHalf(caption.text || caption, UI_CH));
  }
  const aria = "Sequence diagram: " + ps.map(p => p.label).join(", ") + ". " + messages.filter(m => m.label).map(m => `${byId[m.from].label} ${m.label} ${byId[m.to].label}`).join("; ") + ".";
  const lineFor = kind => ({
    stroke: kind === "return" ? "var(--mm-muted)" : "var(--mm-cell-fg)",
    strokeWidth: 1.6,
    fill: "none",
    strokeDasharray: kind === "return" ? "5 4" : "none"
  });
  const headFor = kind => kind === "sync" ? "url(#seq-sync)" : "url(#seq-open)";
  return /*#__PURE__*/React.createElement(DiagramSvg, {
    viewBox: b.viewBox(),
    maxWidth: maxWidth || Math.min(760, Math.round(b.width())),
    ariaLabel: aria
  }, /*#__PURE__*/React.createElement("defs", null, /*#__PURE__*/React.createElement("marker", {
    id: "seq-sync",
    markerWidth: "10",
  ...
  }))), ps.map(p => /*#__PURE__*/React.createElement("line", {
    key: "ll" + p.id,
    x1: p.cx,
    y1: lifeTop(p),
    x2: p.cx,
    ...
    const y0 = msgY(a.from) - pad,
      y1 = msgY(a.to) + pad;
    return /*#__PURE__*/React.createElement("rect", {
      key: "ac" + i,
      x: p.cx - SEQ.AW / 2 + (a.dx || 0),
      y: y0,
      width: SEQ.AW,
      height: y1 - y0,
      style: {
    ...
    const y = msgY(i),
      ...
      B = byId[m.to];
    if (m.from === m.to || m.self) {
      // self-call: a small loop back to the lifeline
      const sx = edge(A, i, 1),
        w = 34;
      return /*#__PURE__*/React.createElement("g", {
        key: "m" + i
      }, /*#__PURE__*/React.createElement("path", {
        d: `M ${sx} ${y - 7} h ${w} v 14 h ${-w}`,
        markerEnd: headFor(m.kind || "sync"),
  ...
  }), ps.map(p => /*#__PURE__*/React.createElement("g", {
    key: "h" + p.id
  }, p.kind === "actor" ? /*#__PURE__*/React.createElement(Actor, {
    x: p.cx,
    y: headTop(p),
    label: p.label
  }) : /*#__PURE__*/React.createElement(SeqObjectBox, {
    label: p.label,
    cx: p.cx,
    top: headTop(p)
  }), p.diesAt != null ? /*#__PURE__*/React.createElement("g", {
    style: {
      stroke: "var(--mm-muted)",
      strokeWidth: 1.8
    }
    ...
    x2: p.cx + 7,
    y2: msgY(p.diesAt) - 7
  })) : null)), annotations.map((a, i) => /*#__PURE__*/React.createElement("g", {
    key: "an" + i
  }, a.to ? /*#__PURE__*/React.createElement("line", {
    x1: a.x,
    y1: a.y + (a.to.y > a.y ? 3 : -9),
    x2: a.to.x,
    y2: a.to.y,
    style: {
      stroke: "var(--mm-muted)",
  ...
  }, a.t))), fragments.flatMap((f, i) => {
    const fx = minX - 30,
      tabW = 34 + (f.kind === "loop" ? 4 : 0);
    const items = [];
    ...
    if (f.guard) items.push({
      x: fx + tabW + 16,
      y: msgY(f.from) - SEQ.BAND - 6 + 14,
      t: `[${f.guard}]`
    });
    (f.dividers || []).forEach(d => {
      if (d.guard) items.push({
        x: fx + 8,
        y: msgY(d.at) - SEQ.BAND - 2 + 14,
        t: `[${d.guard}]`
      });
    });
    return items.map((g, k) => /*#__PURE__*/React.createElement("g", {
      key: "g" + i + "-" + k
    }, /*#__PURE__*/React.createElement("rect", {
      x: g.x - 4,
      y: g.y - 12,
      width: g.t.length * MONO_CH + 8,
      height: 16,
      rx: 2,
      style: {
        fill: "var(--mm-panel-bg)"
...
export function sequenceDiagram(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(SequenceDiagram, cfg);
  };
}
...
export function SequenceOrderBuilder({
  prompt,
  participants = [],
  messages = [],
  activations = []
  ...
  const removeAt = index => {
    if (!revealed && !checked) setPlaced(c => c.filter((_, i) => i !== index));
  };
  const {
    picked,
    chipProps,
  ...
  const stateFor = index => gradedChipState({
    revealed,
    checked,
    filled: true,
    ok: grades ? grades[index].ok : false
  });
  ...
  const status = revealed ? "Revealed — reset to try again." : !checked ? `${placed.length} of ${N} placed` : wrongCount === 0 ? "Correct — that's the interaction." : `${wrongCount} out of place — reset to try again.`;
  const reset = () => {
    setPlaced([]);
    setChecked(false);
    setRevealed(false);
  ...
  const reveal = () => {
    setPlaced(correctIds.slice());
    setChecked(false);
    setRevealed(true);
  };
  return /*#__PURE__*/React.createElement("div", {
    className: "bex bex-seq"
  }, prompt ? /*#__PURE__*/React.createElement("p", {
    className: "bex-prompt"
  }, prompt) : null, /*#__PURE__*/React.createElement("div", {
    className: "bex-seq__cols"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-bank",
    "aria-label": "message bank"
  }, bank.length ? bank.map(id => /*#__PURE__*/React.createElement(Chip, _extends({
    key: id,
    state: picked === id ? "picked" : "idle",
    ariaLabel: `message ${ends(byId[id])} ${byId[id].label}, in bank`
  }, chipProps(id)), ends(byId[id]), " : ", byId[id].label)) : /*#__PURE__*/React.createElement("span", {
    className: "bex-status"
  }, "bank empty")), /*#__PURE__*/React.createElement("ol", {
    className: "bex-slots",
    "aria-label": "ordered messages"
  }, Array.from({
    length: N
  }, (_, i) => {
    const id = placed[i];
    return /*#__PURE__*/React.createElement("li", {
      ...
      tabIndex: 0,
      role: "button",
      "aria-label": `slot ${i + 1}${id ? `: ${byId[id].label}` : ", empty"}`
    }, slotProps(i)), /*#__PURE__*/React.createElement("span", {
      className: "bex-slot__n"
    }, i + 1), id ? /*#__PURE__*/React.createElement(Chip, {
      state: stateFor(i),
      ariaLabel: `message ${byId[id].label} in slot ${i + 1}`,
      onClick: () => removeAt(i)
    }, ends(byId[id]), " : ", byId[id].label, " \u2715") : /*#__PURE__*/React.createElement("span", {
      className: "bex-status"
    ...
    activations: shownActivations
  }) : null, /*#__PURE__*/React.createElement(BuilderControls, {
    status: status,
    revealed: revealed,
...
export function sequenceOrder(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(SequenceOrderBuilder, cfg);
  };
}
...
export function ClassBoxBuilder({
  prompt,
  className = "",
  abstract = false,
  attributes = [],
  ...
  const typeValues = React.useMemo(() => [...attributes.map(a => a.type), ...operations.filter(o => o.ret != null).map(o => o.ret), ...typeDistractors], [attributes, operations, typeDistractors]);
  const typeTokens = React.useMemo(() => seededShuffle(typeValues.map((v, i) => ({
    id: `ty${i}:${v}`,
    value: v
  })), hashSeed(className + "|" + typeValues.join(","))), [typeValues, className]);
  ...
  const pickToken = (pool, value, tokenId) => {
    if (revealed) return;
    setPicked(p => p && p.pool === pool && p.value === value && p.tokenId === tokenId ? null : {
      pool,
      value,
      tokenId
    });
  ...
  const slotState = s => gradedChipState({
    revealed,
    checked,
    filled: !!fills[s.id],
    ok: slotOk(s)
  ...
  const status = revealed ? "Revealed — reset to try again." : !checked ? hasRel ? "Stamp a visibility, drop a type after each colon, pick the relationship — then Check." : "Stamp a visibility and drop a type after each colon — then Check." : correctCount === total ? "Correct — that's the class box." : `${correctCount} of ${total} right — the red marks show what to fix.`;
  const reset = () => {
    setFills({});
    setRel(null);
    setPicked(null);
  ...
  const childOpRows = operations.map((m, i) => m.ret != null ? `${show(`o${i}v`)} ${m.name} : ${show(`o${i}t`)}` : `${show(`o${i}v`)} ${m.name}`);
  const childSections = [{
    rows: childAttrRows
  }, {
    rows: childOpRows
  }];
  const parentSections = relationship.parentSections || [{
    rows: []
  }, {
    rows: []
  }];
  const allRows = [className, relationship.to || "", ...childAttrRows, ...childOpRows, ...parentSections.flatMap(s => (s.rows || []).map(r => typeof r === "object" ? r.text : r))];
  const W = Math.max(150, Math.round(Math.max(...allRows.map(r => String(r).length)) * 6.9) + 26);
  const cx = 14 + W / 2,
    px = 14;
  const parentTop = 8,
    parentH = diagramCardHeight(parentSections, {
      title: true
    });
  const parentBottom = parentTop + parentH,
    childTop = parentBottom + 46;
  const childH = diagramCardHeight(childSections, {
      title: true
    }),
    childBottom = childTop + childH;
  const vb = hasRel ? `0 0 ${W + 28} ${childBottom + 12}` : `0 0 ${W + 28} ${parentTop + childH + 12}`;
  const renderSlot = slot => /*#__PURE__*/React.createElement(Chip, {
  ...
  }, fills[slot.id] ? fills[slot.id].value : /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, slot.kind));
  const whyFor = slotId => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bex-palette",
    "aria-label": "visibility palette"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-palette__label"
  }, "Visibility"), CBX_VIS.map(v => /*#__PURE__*/React.createElement(Chip, {
    key: v,
    state: picked && picked.pool === "vis" && picked.value === v ? "picked" : "idle",
    ariaLabel: `visibility ${v}`,
    draggable: !revealed,
    onDragStart: e => {
      setPicked({
        pool: "vis",
  ...
  }, v))), !revealed ? /*#__PURE__*/React.createElement("div", {
    className: "bex-typebank",
    "aria-label": "type bank"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-typebank__label"
  }, "Types"), bank.length ? bank.map(t => /*#__PURE__*/React.createElement(Chip, {
    key: t.id,
    state: picked && picked.tokenId === t.id ? "picked" : "idle",
    ariaLabel: `type ${t.value}`,
    draggable: true,
    onDragStart: e => {
      setPicked({
        pool: "type",
    ...
    onKeyDown: e => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        pickToken("type", t.value, t.id);
      }
    }
  }, t.value)) : /*#__PURE__*/React.createElement("span", {
    className: "bex-status"
  }, "every type placed")) : null, /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx-wrap"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx-build"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-zone-label"
  }, "Build the class box"), /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx" + (abstract ? " bex-cbx--abstract" : ""),
    "aria-label": `class box for ${className}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx__name" + (abstract ? " bex-cbx__name--abstract" : "")
  }, className), /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx__sect"
  }, attributes.length ? attributes.map((m, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "bex-cbx__row"
  }, renderSlot(slots.find(s => s.id === `a${i}v`)), /*#__PURE__*/React.createElement("span", null, m.name), /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__colon"
  }, ":"), renderSlot(slots.find(s => s.id === `a${i}t`)), whyFor(`a${i}v`), whyFor(`a${i}t`))) : /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx__row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, "no attributes"))), /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx__sect"
  }, operations.length ? operations.map((m, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "bex-cbx__row"
  }, renderSlot(slots.find(s => s.id === `o${i}v`)), /*#__PURE__*/React.createElement("span", null, m.name), m.ret != null ? /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__colon"
  }, ":") : null, m.ret != null ? renderSlot(slots.find(s => s.id === `o${i}t`)) : null, whyFor(`o${i}v`), whyFor(`o${i}t`))) : /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx__row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, "no operations")))), hasRel ? /*#__PURE__*/React.createElement("div", {
    className: "bex-cbx-rel"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-rel-title"
  }, "Relationship: ", /*#__PURE__*/React.createElement("code", null, className), " to ", /*#__PURE__*/React.createElement("code", null, relationship.to)), /*#__PURE__*/React.createElement("div", {
    className: "bex-rel",
    role: "group",
    "aria-label": "relationship kind"
  }, CBX_RELS.map(r => /*#__PURE__*/React.createElement(Chip, {
    key: r.kind,
    state: relState(r.kind),
    ariaLabel: r.name,
    ...
    className: "bex-zone-label"
  }, "Preview"), /*#__PURE__*/React.createElement(DiagramSvg, {
    viewBox: vb,
    maxWidth: Math.min(340, W + 28),
    ariaLabel: hasRel ? `class diagram preview: ${className} and ${relationship.to}` : `class diagram preview: ${className}`
  }, hasRel ? rel ? /*#__PURE__*/React.createElement(UmlLink, {
    from: {
      x: cx,
      y: childTop
    },
  ...
  }) : null, hasRel ? /*#__PURE__*/React.createElement(DiagramCard, {
    x: px,
    y: parentTop,
    w: W,
    ...
    neutral: true,
    abstract: relationship.parentAbstract
  }) : null, /*#__PURE__*/React.createElement(DiagramCard, {
    x: px,
    y: hasRel ? childTop : parentTop,
    w: W,
    title: className,
...
export function classBuild(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(ClassBoxBuilder, cfg);
  };
}
...
export function MatchBuilder({
  ...
  options = [],
  items = [],
  paletteLabel = "Principles",
  slotLabel = "violates",
  slotPlaceholder = "principle"
}) {
  ...
  const chipStateAt = i => gradedChipState({
    revealed,
    checked,
    filled: fills[i] != null,
    ok: okAt(i)
  ...
  const status = revealed ? "Revealed — reset to try again." : !checked ? `${filledCount} of ${items.length} matched` : correctCount === items.length ? "Correct — every scenario matched." : `${correctCount} of ${items.length} right — reset to try again.`;
  const reset = () => {
    setFills({});
    setPicked(null);
    setChecked(false);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bex-palette__label"
  }, paletteLabel), opts.map(o => /*#__PURE__*/React.createElement(Chip, {
    key: o.value,
    state: picked === o.value ? "picked" : "idle",
    ariaLabel: o.label,
    draggable: !locked,
    onDragStart: e => {
      setPicked(o.value);
    ...
    key: i,
    className: "bex-match__item"
  }, it.code ? /*#__PURE__*/React.createElement(CodeBlock, {
    code: it.code,
    lang: it.lang || "java"
  }) : null, it.text ? /*#__PURE__*/React.createElement("div", {
    className: "bex-match__scene"
  }, renderCaption(it.text)) : null, /*#__PURE__*/React.createElement("div", {
    className: "bex-match__row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-match__label"
  }, slotLabel), /*#__PURE__*/React.createElement(Chip, {
    state: chipStateAt(i),
  ...
  }, fills[i] != null ? labelOf(fills[i]) : /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, slotPlaceholder))), checked && fills[i] != null && !okAt(i) && it.why ? /*#__PURE__*/React.createElement("div", {
    className: "bex-why"
  }, renderCaption(it.why)) : null))), /*#__PURE__*/React.createElement(BuilderControls, {
    status: status,
    revealed: revealed,
...
export function matchBuild(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(MatchBuilder, cfg);
  };
}
...
function clusterByRelations(ids, relations) {
  ...
  ids.forEach(id => {
    ...
    while (stack.length) {
      ...
      seen.add(x);
      out.push(x);
      adj.get(x).filter(n => !seen.has(n)).sort((a, b) => deg(a) - deg(b) || pos[a] - pos[b]) // walk toward the hub, then out
...
export function UseCaseBuilder({
  ...
  const unreveal = (...names) => setRevealedSet(s => {
    const n = new Set(s);
  ...
  const cycleTag = i => {
    ...
    setTags(m => {
      ...
      if (next) n[i] = next;else delete n[i];
      return n;
    });
  };
  const roleWord = r => r === "case" ? "use case" : "actor"; // only called with a set tag
  const idWhy = s => s.why || (s.role === "actor" ? `“${s.w}” is an actor — someone outside the system who uses it.` : s.role === "case" ? `“${s.w}” is a use case — a goal the system carries out for an actor.` : `“${s.w}” is neither an actor nor a use case here — leave it untagged.`);
  ...
  const connect = id => {
    ...
    const za = zoneOf(picked),
      zb = zoneOf(id);
    if (za === "actors" && zb === "system") {
      toggleAssoc(picked, id);
      setPicked(null);
    } else if (za === "system" && zb === "actors") {
      toggleAssoc(id, picked);
      setPicked(null);
    } else if (za === "system" && zb === "system") {
      setPendingRel({
        from: picked,
        to: id
      });
  ...
  const toggleAssoc = (actor, c) => {
    dirty();
    setAssoc(a => a.includes(ucKey(actor, c)) ? a.filter(x => x !== ucKey(actor, c)) : [...a, ucKey(actor, c)]);
  };
  const addRel = kind => {
    ...
    setRels(r => {
      ...
      return [...r.filter(x => !(x.from === from && x.to === to)), {
        from,
        to,
        kind
      }];
  ...
  const previewActors = inActors.map(id => ({
    id,
    label: byId[id].label,
    side: "left"
  }));
  ...
  const sysSet = new Set(inSystem),
    actSet = new Set(inActors);
  const previewAssoc = React.useMemo(() => {
    const g = {};
    assoc.forEach(k => {
      const [a, c] = k.split("|");
      if (actSet.has(a) && sysSet.has(c)) (g[a] = g[a] || []).push(c);
    });
    return Object.entries(g).map(([actor, cases]) => ({
      actor,
      cases
    }));
  }, [assoc, zone]);
  ...
  const chipState = id => {
    ...
    return gradedChipState({
      revealed: false,
      checked: checked && stage === "place",
      filled: id in zone,
      ok: zoneRight(byId[id])
    });
  };
  ...
  const reveal = () => {
    if (stage === "identify") {
      const t = {};
      spans.forEach(s => {
        if (s.role !== "none") t[s.i] = s.role;
      });
      setTags(t);
      setRevealedSet(s => new Set(s).add("identify"));
    } else if (stage === "place") {
      const z = {};
      elements.forEach(e => {
        z[e.id] = wantZone(e);
      ...
      setZone(z);
      setPlaceViaConnect(false);
      setRevealedSet(s => new Set(s).add("place"));
    } else {
      const z = {};
      elements.forEach(e => {
        z[e.id] = wantZone(e);
      ...
      setZone(z);
      setAssoc([...authoredAssoc]);
      setRels(relations.map(r => ({
        from: r.from,
        to: r.to,
        kind: r.kind
      })));
  ...
  const renderChip = (id, onClick) => /*#__PURE__*/React.createElement(Chip, {
    key: id,
    state: chipState(id),
    ariaLabel: `${byId[id].label} (${byId[id].role})`,
    onClick: onClick,
    onKeyDown: onEnterOrSpace(onClick)
  }, joinLabel(byId[id].label));
  const chipClick = id => stage === "place" ? () => {
    if (revealed) return;
    if (id in zone) toBank(id);else setPicked(p => p === id ? null : id);
  } : () => connect(id);
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "bex bex-uc-ex"
  }, prompt ? /*#__PURE__*/React.createElement("p", {
    className: "bex-prompt"
  }, prompt) : null, /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-stages",
    role: "group",
    "aria-label": "build stage"
  }, hasIdentify ? /*#__PURE__*/React.createElement(Chip, {
    state: stage === "identify" ? "picked" : "idle",
    ariaLabel: "identify stage",
    onClick: () => {
      setPicked(null);
      setChecked(false);
      setStage("identify");
    }
  }, "1 \xB7 Identify") : null, /*#__PURE__*/React.createElement(Chip, {
    state: stage === "place" ? "picked" : "idle",
    ariaLabel: "place stage",
    onClick: () => {
      setPicked(null);
      setChecked(false);
      setStage("place");
    }
  }, hasIdentify ? "2" : "1", " \xB7 Place"), /*#__PURE__*/React.createElement(Chip, {
    state: stage === "connect" ? "picked" : "idle",
    ariaLabel: "connect stage",
    onClick: () => {
      setPicked(null);
      setChecked(false);
      setStage("connect");
    }
  }, hasIdentify ? "3" : "2", " \xB7 Connect")), hasIdentify && stage === "identify" ? /*#__PURE__*/React.createElement("div", {
    className: "uc-id"
  }, /*#__PURE__*/React.createElement("p", {
    className: "uc-id__sentence",
    "aria-label": "requirements to tag"
  }, spans.map(s => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, {
      ...
      className: "uc-id__tag",
      "aria-hidden": "true"
    }, tag ? roleWord(tag) : ""), /*#__PURE__*/React.createElement("span", {
      className: "uc-id__word"
    }, s.w)), " ");
  }))) : null, stage === "place" && !revealed && bank.length ? /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-bank",
    "aria-label": "element bank"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-palette__label"
  }, "Unplaced"), bank.map(id => renderChip(id, chipClick(id)))) : null, stage !== "identify" ? /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-zones"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-gutter",
    role: "button",
    tabIndex: 0,
    "aria-label": "actor gutter, outside the system",
    onClick: () => stage === "place" && placeInZone("actors"),
    onKeyDown: e => {
      if (stage === "place" && (e.key === "Enter" || e.key === " ")) {
        e.preventDefault();
        placeInZone("actors");
      }
    }
  }, /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-zone__label"
  }, "Actors (outside)"), inActors.map(id => renderChip(id, chipClick(id))), !inActors.length ? /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, "drop actors here") : null), /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-system",
    role: "button",
    tabIndex: 0,
    ...
    className: "bex-uc-zone__label"
  }, system, " (inside)"), inSystem.map(id => renderChip(id, chipClick(id))), !inSystem.length ? /*#__PURE__*/React.createElement("span", {
    className: "bex-cbx__ph"
  }, "drop use cases here") : null)) : null, stage === "connect" && pendingRel && !revealed ? /*#__PURE__*/React.createElement("div", {
    className: "bex-rel",
    role: "group",
    "aria-label": "relation kind"
  }, /*#__PURE__*/React.createElement("span", {
    className: "bex-palette__label"
  }, byId[pendingRel.from].label, " \u2192 ", byId[pendingRel.to].label, ":"), UC_RELS.map(r => /*#__PURE__*/React.createElement(Chip, {
    key: r.kind,
    state: "idle",
    ariaLabel: r.label,
    onClick: () => addRel(r.kind)
  }, r.label)), /*#__PURE__*/React.createElement(Chip, {
    state: "idle",
    ariaLabel: "cancel relation",
    onClick: () => setPendingRel(null)
  }, "cancel")) : null, previewActors.length || previewCases.length ? /*#__PURE__*/React.createElement(UseCaseDiagram, {
    system: system,
    actors: previewActors,
    cases: previewCases,
    associations: previewAssoc,
    relations: previewRels,
    maxWidth: 520,
    rowGap: 96,
    onPick: revealed ? undefined : id => chipClick(id)(),
    pickedId: picked
  }) : null, checked && !revealed && !stageInfo.ok ? /*#__PURE__*/React.createElement("div", {
    className: "bex-uc-fb"
  }, stage === "identify" ? spans.filter(s => !spanRight(s)).map(s => /*#__PURE__*/React.createElement("div", {
    key: "id" + s.i,
    className: "bex-why"
  }, idWhy(s))) : null, stage === "place" ? wrongZoned.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
    className: "bex-why"
  }, joinLabel(e.label), ": ", e.whyZone || whyZone[e.role] || (e.role === "actor" ? "An actor sits OUTSIDE the boundary." : "A use case sits INSIDE the boundary."))) : null, stage === "connect" ? /*#__PURE__*/React.createElement(React.Fragment, null, missingAssoc.length || extraAssoc.length ? /*#__PURE__*/React.createElement("div", {
    className: "bex-why"
  }, "Associations: ", missingAssoc.length, " missing, ", extraAssoc.length, " extra. An actor joins each use case it takes part in.") : null, missingRels.map((r, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "bex-why"
  }, byId[r.from].label, " \u2192 ", byId[r.to].label, ": ", r.why || `should be «${r.kind}».`)), extraRels.map((r, i) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: "x" + i,
      className: "bex-why"
    }, byId[r.from].label, " \u2192 ", byId[r.to].label, ": ", reversed ? `right relation, wrong way round — the ${relName} arrow runs ${byId[r.to].label} → ${byId[r.from].label}.` : `this ${relName} is not part of the diagram — remove it.`);
  })) : null) : null, /*#__PURE__*/React.createElement(BuilderControls, {
    status: status,
    revealed: revealed,
    checkDisabled: revealed || (stage === "identify" ? Object.keys(tags).length === 0 : stage === "place" ? Object.keys(zone).length === 0 : assoc.length === 0 && rels.length === 0),
    onCheck: () => setChecked(true),
    onReset: reset,
    onReveal: reveal
  }));
}
export function useCaseBuild(cfg) {
  return function App() {
    return /*#__PURE__*/React.createElement(UseCaseBuilder, cfg);
  };
}
...
export function MemoryCompare({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "mm-scene mm-cmp-wrap"
  }, title ? /*#__PURE__*/React.createElement("div", {
    className: "mm-scene__title",
    "data-artifact-title": true
  }, title) : null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "mm-cmp__col",
    key: i
  }, /*#__PURE__*/React.createElement("code", {
    className: "mm-cmp__code"
  }, highlight(st.code, st.lang || lang)), /*#__PURE__*/React.createElement(MemoryModel, {
    cells: st.cells,
    axis: false,
    regions: ["stack"],
    legend: false
  }), st.tag ? /*#__PURE__*/React.createElement("span", {
    className: "mm-cmp__tag mm-cap-tag mm-cap-tag--" + (st.tag.kind || "cpp")
  }, st.tag.text) : null, st.note ? /*#__PURE__*/React.createElement("p", {
    className: "mm-cmp__note"
  }, renderCaption(st.note)) : null))), punch || hint ? /*#__PURE__*/React.createElement("div", {
    className: "mm-cmp__foot"
  }, punch ? /*#__PURE__*/React.createElement("p", {
    className: "mm-cmp__punch"
  }, renderCaption(punch)) : null, hint ? /*#__PURE__*/React.createElement("p", {
    className: "mm-cmp__hint"
  }, hint) : null) : null);
}
...
export function sizes(config) {
  return function App() {
    return /*#__PURE__*/React.createElement(SizeCompare, config);
  };
}
export function compare(config) {
  return function App() {
    return /*#__PURE__*/React.createElement(MemoryCompare, config);
  };
}
export function dualScene(config) {
  return function App() {
    return /*#__PURE__*/React.createElement(MemoryDualScene, config);
  };
}
...
export function l06VtableScene({
  predict
} = {}) {
...
p = nullptr;`;
  ...
  const steps = [{
    ...
    line: 4,
    cells: [P("nullptr", true), H("42", true)],
    caption: {
