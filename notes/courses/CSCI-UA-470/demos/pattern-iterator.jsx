...
const GOOD = patternTree({
  contextW: 218, gapX: 48, edge: "assoc", edgeLabel: "walks",
  context: { title: "Collection", sections: [
    { rows: ["- items : List<Item>"] },
    { rows: ["+ addItem(i)"] },
  ]},
  parent: { title: "Iterator", abstract: true, sections: [
    { rows: [ab("+ hasNext() : boolean"), ab("+ getNext() : Item")] },
  ]},
  children: ["SizeIterator", "AgeIterator", "HeightIterator"].map((t) => ({
    title: t, sections: [{ rows: ["+ hasNext() : boolean", "+ getNext() : Item"] }],
  })),
  cardW: 176, gap: 16,
...
while (i.hasNext()) process(i.getNext());`,
  ...
  caption: {
    cols: [
      ...
      { tag: "iterator", kind: "int", children: <>Holds the <strong>cursor</strong> — which is why two iterators can walk the same collection at once, something a method on <code className="ui-ic">Collection</code> could never manage.</> },
    ],
