...
const REGIONS = [
  { key: "method",  label: "Method Area",   hint: "class blueprints" },
  { key: "heap",    label: "Heap",          hint: "objects" },
  { key: "opstack", label: "Operand Stack", hint: "per-frame work" },
  { key: "pc",      label: "PC",            hint: "current opcode" },
];
...
const point = obj("Point", [{ name: "x", type: "int" }, { name: "y", type: "int" }],
  { region: "heap", header: 12 });
const BP  = (hl) => methodArea("Point", "class", "{ x:int, y:int }", { id: "blueprint", hl });
const OBJ = (x, y, hl, opts = {}) => point("pointobj", { x, y }, { addr: "heap@0x100", hl, ...opts });
const REF = (hl) => opstack("operand[top]", "ref", "Point@0x100", { id: "ref", to: "pointobj", link: "ref", hl });
const PC  = (txt, hl) => pcreg("PC", "opcode", txt, { id: "pc", hl });
...
const steps = [
  ...
  },
  {
    ...
    outputs: [{ expr: "DESCRIBE", result: "Point{x=3, y=4}" }],
    caption: {
  ...
  {
    active: ["heap"],
    cells: [BP(), OBJ(3, 4, false, { reclaimed: true }), PC("(reference dropped)", true)],
    caption: {
