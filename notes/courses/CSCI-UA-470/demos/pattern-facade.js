...
const subH = Math.max(...SUB.map(s => diagramCardHeight(s.sections, {
  title: true
})));
const rowW = SUB.length * CARD_W + (SUB.length - 1) * GAP;
const facH = diagramCardHeight(FACADE.sections, {
  title: true
});
...
w1.deliver(f);`,
  ...
  good: {
    ...
    }, "SUBSYSTEM \u2014 UNCHANGED"), SUB.map((s, i) => /*#__PURE__*/React.createElement(DiagramCard, {
      key: s.title,
      x: PAD + i * (CARD_W + GAP),
      y: PAD + 16,
      w: CARD_W,
    ...
    }, "the facade \u2014 the one class that is new"))
  },
  client: {
    lang: "java",
    label: "client code",
