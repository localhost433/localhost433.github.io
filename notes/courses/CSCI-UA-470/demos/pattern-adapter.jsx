...
const child = (title, attrs = []) => ({ title, sections: [{ rows: attrs }, { rows: ops }] });

const PAD = 14, CARD_W = 158, GAP = 14, DROP = 44;
const L = treeLayout({
  cx: 0, topY: PAD, cardW: CARD_W, gap: GAP,
  parent: { title: "DBCNN", abstract: true,
    sections: [{ rows: ["- dbName : String"] }, { rows: ops } ] },
  children: [
    child("OracleCNN"), child("SQLServerCNN"), child("MySQLCNN"),
    child("SQLiteCNN", ["- s : SQLite"]),
  ],
});
const SHIFT = PAD - L.left;
const A = L.children[3];                       // the adapter — everything below hangs off it
const adapterCx = A.cx + SHIFT, adapterBottom = A.top + A.h;

const SQLITE = { title: "SQLite",
  sections: [{ rows: ["- dbName : String"] }, { rows: ["+ command(q)", "+ loadData() : Data"] }] };
const sqliteY = adapterBottom + DROP;
const sqliteH = diagramCardHeight(SQLITE.sections, { title: true });
...
const GOOD = {
  width: W, height: H, viewBox: `0 0 ${W} ${H}`, maxWidth: 820,
  ...
  node: (
    <g transform={`translate(${SHIFT}, 0)`}>
      <ClassTree layout={L} />
      <g transform={`translate(${-SHIFT}, 0)`}>
        <UmlLink from={{ x: adapterCx, y: adapterBottom }} to={{ x: adapterCx, y: sqliteY }}
          kind="assoc" label="wraps" />
        <DiagramCard x={sqliteX} y={sqliteY} w={CARD_W} title={SQLITE.title} sections={SQLITE.sections} sub={2} />
        <SvgCode x={bodyX} y={sqliteY} lines={BODY} title="SQLiteCNN.read(q)" />
      </g>
    </g>
  ),
};
