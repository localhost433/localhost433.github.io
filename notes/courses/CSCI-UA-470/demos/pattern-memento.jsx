...
// ctrl+Z once more -> whatever was saved before that`,
  ...
  caption: {
    cols: [
      { tag: "originator", kind: "cpp", children: <>The <code className="ui-ic">Document</code> holds the state and only that. It never learns that it is being snapshotted, so undo depth is not its problem.</> },
      { tag: "caretaker", kind: "int", children: <>The canonical <code className="ui-ic">CareTaker</code> stores the stack and does not interpret it. This deck's simplified version exposes the snapshot fields and copies them directly in <code className="ui-ic">undo()</code>.</> },
    ],
