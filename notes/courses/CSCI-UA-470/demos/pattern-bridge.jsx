...
const A = treeLayout({
  ...
  parent: { title: "computer", abstract: true,
    sections: [{ rows: ["- cpu : String", "- RAM : String", "- os : OS"] }] },
  children: [leaf("Pc"), leaf("Laptop"), leaf("desktop")],
});
const B = treeLayout({
  cx: 0, topY: PAD, cardW: CARD_W, gap: GAP,
  parent: { title: "OS", abstract: true, sections: [{ rows: osOps }] },
...
const label = (x, y, text) => (
  <text x={x} y={y} textAnchor="middle"
    style={{ fill: "var(--mm-muted)", fontSize: 11, fontWeight: 700, letterSpacing: ".05em" }}>{text}</text>
);
...
// shutdown() bug has to be fixed in three of them.`,
  ...
  good: {
    ...
    node: (
      <g>
        {label(A.parent.cx + aShift, PAD - 2, "ABSTRACTION")}
        {label(B.parent.cx + bShift, PAD - 2, "IMPLEMENTATION")}
        <g transform={`translate(${aShift}, 12)`}><ClassTree layout={A} /></g>
        <g transform={`translate(${bShift}, 12)`}><ClassTree layout={B} /></g>
        <UmlLink kind="assoc" label="the bridge"
          from={{ x: A.parent.cx + aShift + CARD_W / 2, y: A.parent.y + 12 + A.parent.h / 2 }}
          to={{ x: B.parent.cx + bShift - CARD_W / 2, y: B.parent.y + 12 + B.parent.h / 2 }} />
