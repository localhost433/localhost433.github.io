...
export default mcq({
  questions: [{
    ...
    choices: [{
      text: "C++ only — `new` there allocates **any** type and returns a pointer; Java's `new` creates **objects** only (primitives are not `new`-ed)",
      correct: true
    }, {
    ...
    choices: [{
      text: "`class` members default to **private**; `struct` members default to **public** — that default is the only real difference between them",
      correct: true
    }, {
    ...
    choices: [{
      text: "Java has static **fields** (class-level members); what it lacks is C++-style **local static variables** inside a function",
      correct: true
    }, {
    ...
    choices: [{
      text: "When derived objects are deleted through a **base pointer** — without a virtual destructor, `~Derived` is skipped (undefined behavior)",
      correct: true
    }, {
    ...
    choices: [{
      text: "A C++ reference is an **alias** — it must be initialized and cannot be reseated or null; a Java reference is a pointer-like handle that can be `null` and reassigned",
      correct: true
    }, {
    ...
    choices: [{
      text: "C++ has `friend` (Java does not); Java arrays are **objects that know their length** (raw C++ arrays carry no length metadata)",
      correct: true
    }, {
