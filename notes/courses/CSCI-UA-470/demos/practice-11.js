/* AUTO-GENERATED from practice-11.jsx by `npm run build:artifacts` — do not edit. */
...
const OP = (id, val, hl) => stack(id, "operand", val, {
  id,
  hl
});
const METHOD = (state, hl) => stack("m", "hot()", state, {
  id: "m",
  hl
});
...
const jitScene = {
  ...
  steps: k => k.calls === "hot" ? [{
    line: 1,
    cells: [METHOD("JIT-compiled → native", true)],
    caption: {
      jvm: "Past the hotness threshold the JIT compiles `hot()` to **native code**; later calls run the compiled version.",
      intuition: "Hot methods stop being interpreted and start running as native machine code."
    }
  }] : [{
    line: 1,
    cells: [METHOD(k.calls === "warm" ? "interpreted (warming)" : "interpreted", true)],
    caption: {
      jvm: k.calls === "warm" ? "~1k calls: still **interpreted**, but the counter is climbing toward the threshold." : "A cold method runs in the **interpreter**, one bytecode at a time.",
...
export default quiz({
  items: [{
    ...
    goal: k => k.calls === "hot",
    success: {
      why: "Once a method is called enough times it becomes **hot**; the JIT compiles it to native code so it no longer runs in the interpreter."
    }
  }]
});