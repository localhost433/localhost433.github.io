...
const X = (name, hl) => stack(name, "int", "5", { id: "x", hl });
const P = (hl) => stack("p", "int*", "&x", { id: "p", to: "x", hl });
...
const asmMap = {
  1: [3],     // int x = 5;    -> mov dword [x], 5
  2: [4],     // int& r = x;   -> no-code note row
  3: [5, 6],  // int* p = &x;  -> lea + mov
};

const asmLabel = "x86-64 · Intel";

export default scene({ title: "Reference vs pointer: an alias has no storage of its own", code, steps, asm, asmMap, asmLabel });
