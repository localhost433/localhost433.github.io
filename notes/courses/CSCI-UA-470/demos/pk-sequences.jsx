...
const FLOWS = {
  add: {
    ...
    participants: [
      ...
      { id: "p", label: "p : Password", bornAt: 1 },
      { id: "f", label: "f : FileManager", bornAt: 3 },
    ],
    messages: [
    ...
    caption: { text: "p joins the list and stays; f is built, used once, and dropped — remember that difference", color: "--mm-muted" },
  },
  view: {
    label: "View passwords",
    participants: [
      { id: "user", label: "User", kind: "actor" },
      { id: "mgr", label: "mgr : MainGUI", bornAt: 0 },
    ...
    activations: [
      { p: "mgr", from: 1, to: 4 },
      { p: "mgr", from: 1, to: 1, dx: 4 },
      { p: "f", from: 3, to: 4 },
    ],
    caption: { text: "viewing IS startup: the constructor ends by calling load_passwords()", color: "--mm-muted" },
  },
  delete: {
    ...
    caption: { text: "the mirror of Add: remove instead of append, then the same fresh FileManager and save", color: "--mm-muted" },
  },
};

const KNOBS = [{
  id: "flow", label: "use case",
  options: [
    { value: "add", label: "Add password" },
    { value: "view", label: "View passwords" },
  ...
  return (
    <div className="mm-scene">
      <div className="mm-scene__title" data-artifact-title>
        The three use cases as interactions — {f.label}
      </div>
      <KnobBar knobs={KNOBS} value={{ flow }} onChange={(_, v) => setFlow(v)} />
      <SequenceDiagram
