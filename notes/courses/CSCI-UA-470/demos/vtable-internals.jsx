...
const PIN  = (hl) => text("Person::intro",  "fn", "cout << \"person\"",  { id: "p_intro", hl });
const PEAT = (hl) => text("Person::eat",    "fn", "cout << \"nom\"",     { id: "p_eat",   hl });
const SIN  = (hl) => text("Student::intro", "fn", "cout << \"student\"", { id: "s_intro", hl });
...
const PVT = (hl) => glob("Person::vtable", "vtable", "", { id: "pvt", hl, fields: [
  { name: "#0 intro", size: 8, to: "p_intro", value: "&Person::intro" },
  { name: "#1 eat",   size: 8, to: "p_eat",   value: "&Person::eat" },
]});
const SVT = (hl) => glob("Student::vtable", "vtable", "", { id: "svt", hl, fields: [
  { name: "#0 intro", size: 8, to: "s_intro", value: "&Student::intro" },   // overridden -> Student's body
  { name: "#1 eat",   size: 8, to: "p_eat",   value: "&Person::eat" },       // inherited -> same Person body
]});

// the Student object on the heap; its hidden vptr points at Student::vtable.
const studentObj = obj("Student", [{ name: "name", type: "string", size: 32 }], { vptr: "svt", region: "heap" });
const OBJ = (hl) => studentObj("*p", ['"James"'], { hl });
...
const asmMap = {
  4:  [1, 2],               // virtual void intro()  -> Person::vtable label + slot #0
  5:  [3],                  // virtual void eat()    -> slot #1
  9:  [4, 5, 6],            // void intro() override -> Student::vtable
  13: [9, 10, 11, 12, 13],  // Person* p = new Student();  (allocate + install vptr)
  14: [14, 15],             // p->eat();   load vptr, index slot #1, call
  15: [16, 17],             // p->intro(); load vptr, index slot #0, call
};

const asmLabel = "x86-64 · clang -O1 · Intel, demangled";

export default scene({ title: "Inside the vtable: an array indexed by a compile-time slot offset", code, steps, asm, asmMap, asmLabel });
