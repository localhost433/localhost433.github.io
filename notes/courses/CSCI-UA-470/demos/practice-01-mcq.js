...
export default mcq({
  questions: [{
    ...
    choices: [{
      text: "True — `a = b` assigns `3` to `a`, and the `if` tests that value (`3`, non-zero)",
      correct: true
    }, {
    ...
    choices: [{
      text: "Always true — it parses as `(3 < a) < 8`, i.e. `1 < 8`",
      correct: true
    }, {
    ...
    choices: [{
      text: "Any other function: undefined behavior (garbage from the return register). `main` is special: falling off it is defined as `return 0`",
      correct: true
    }, {
    ...
    choices: [{
      text: "No — `long` is 32-bit on 64-bit Windows but 64-bit on 64-bit Linux; use `long long` (min 64 bits) or check `sizeof`",
      correct: true
    }, {
      text: "Yes — `long` is always 8 bytes on any 64-bit platform"
    }, {
      text: "Yes — the standard fixes `long` at exactly 32 bits everywhere"
    }, {
    ...
    stem: "`name` below is a **C-string** (`char[]`). Which statement is true?",
    figure: {
      code: "char a[10] = \"HELLO\";\nchar b[10];\nb = a;          // (1)\nif (a == b) {}  // (2)",
      lang: "cpp"
    },
    choices: [{
      text: "Both are wrong: `=` copies a C-string only at declaration (1 fails), and `==` compares addresses, not contents (2)",
      correct: true
    }, {
      text: "Both work: `=` copies the characters and `==` compares them"
    }, {
      text: "(1) works but (2) compares addresses"
    }, {
      text: "(2) works but (1) is a compile error"
    }],
