...
const RA = () => heap("", "int", "0", { id: "ra" });
const RB = () => heap("", "int", "0", { id: "rb" });
  ...
  steps: [
    {
      ...
      caption: { cpp: "`Circle a;` allocates one heap `int`; `a.radius` points at it.",
    ...
    },
    {
      ...
      predict: {
        ...
        choices: [
          { label: "One — `a` and `b` share it", correct: true },
      ...
      caption: { cpp: "Members are copied bitwise, so `b.radius` holds the **same address** as `a.radius`.",
...
const copyGoal = {
  ...
  knobs: [
    { id: "copy", label: "Copy mode", options: [
      { value: "shallow", label: "shallow (default)" },
      { value: "deep",    label: "deep (copy ctor)" },
    ], default: "shallow" },
  ],
  steps: (k) => k.copy === "deep"
    ? [ { line: 1, cells: [circle("a", "ra", true), circle("b", "rb", true), RA(), RB()],
          caption: { cpp: "`radius = new int(*o.radius)` — `b` gets its **own** heap block.",
                     intuition: "Two allocations → each destructor frees a different block." } } ]
    : [ { line: 1, cells: [circle("a", "ra"), circle("b", "ra", true), RA()],
          caption: { cpp: "Shallow (default) copy: `a` and `b` hold the **same** address.",
                     intuition: "Both destructors free the same block — a double free." } } ],
};

export default quiz({ items: [
  { kind: "predict", scene: copyPredict },
  { kind: "goal", scene: copyGoal,
    prompt: "Set the **copy mode** so that `a` and `b` each own a **separate** heap block (no double free).",
    goal: (k) => k.copy === "deep",
    success: { why: "A **deep** copy allocates a fresh `int` per object, so each destructor frees a different block. This is the Rule of Three motivation." } },
] });
