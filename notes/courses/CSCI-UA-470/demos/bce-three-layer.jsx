...
const SYS = { x: 150, y: 40, w: 560, h: 280 };
const CTRL = { cx: 455, cy: 195 };
const DBB = { cx: 650, cy: 180 };
const ENTY = 96; // entity row y
const entities = [350, 405, 460, 515, 570];

const legend = [
  { label: "boundary", sub: 0, x: 220 },
  { label: "control", sub: 2, x: 350 },
  { label: "entity", sub: 1, x: 470 },
];

export default function BceThreeLayer() {
  return (
    <DiagramSvg viewBox="0 0 880 400" maxWidth={760}
      ...
      {/* system box */}
      <rect x={SYS.x} y={SYS.y} width={SYS.w} height={SYS.h} rx={10}
        style={{ fill: "none", stroke: "var(--mm-cell-bd)", strokeWidth: 1.5 }} />
      <text x={SYS.x + 10} y={SYS.y + 20} style={{ fill: "var(--mm-muted)", fontSize: 11, fontWeight: 700 }}>System</text>

      {/* layer slices */}
      <line x1={318} y1={SYS.y + 8} x2={318} y2={SYS.y + SYS.h - 8}
        style={{ stroke: "var(--mm-gap-bd)", strokeWidth: 1, strokeDasharray: "3 4" }} />
      <line x1={600} y1={SYS.y + 8} x2={600} y2={SYS.y + SYS.h - 8}
        style={{ stroke: "var(--mm-gap-bd)", strokeWidth: 1, strokeDasharray: "3 4" }} />

      {/* user side */}
      <Actor x={52} y={130} label="user" />
      <DiagramEdge from={{ x: 84, y: 160 }} to={{ x: 186, y: 130 }} label="input" />
      <DiagramEdge from={{ x: 186, y: 230 }} to={{ x: 84, y: 200 }} label="output" />

      {/* boundary objects on the user edge */}
      <DiagramBox cx={240} cy={120} w={92} h={34} label="LoginForm" sub={0} />
      <DiagramBox cx={240} cy={240} w={92} h={34} label="ReportView" sub={0} />

      {/* control object */}
      <DiagramBox cx={CTRL.cx} cy={CTRL.cy} w={92} h={38} label="Handler" note="control" sub={2} />
      <DiagramEdge from={{ x: 286, y: 130 }} to={{ x: 409, y: 185 }} />
      <DiagramEdge from={{ x: 409, y: 207 }} to={{ x: 286, y: 232 }} />

      {/* entity fan-out */}
      {entities.map((ex, i) => (
        <DiagramEdge key={i} from={{ x: CTRL.cx + 6 * (i - 2), y: CTRL.cy - 19 }} to={{ x: ex, y: ENTY + 14 }} />
      ))}
      {entities.map((ex, i) => (
        <DiagramBox key={i} cx={ex} cy={ENTY} w={48} h={28} label={"e" + (i + 1)} sub={1} />
      ))}
      <text x={460} y={62} textAnchor="middle" style={{ fill: "var(--mm-muted)", fontSize: 10.5, fontStyle: "italic" }}>
        entity objects — the data
      </text>

      {/* DB-connection boundary on the DBMS edge */}
      ...
      {legend.map((l) => (
        <g key={l.label}>
          <DiagramBox cx={l.x} cy={352} w={16} h={16} label="" sub={l.sub} />
          <text x={l.x + 14} y={352} dominantBaseline="central"
            style={{ fill: "var(--mm-muted)", fontSize: 11 }}>{l.label}</text>
        </g>
      ))}
      <text x={430} y={386} textAnchor="middle" style={{ fill: "var(--mm-muted)", fontSize: 10.5, fontStyle: "italic" }}>
