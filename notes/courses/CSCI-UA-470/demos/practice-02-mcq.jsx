  ...
  questions: [
    {
      ...
      figure: { code: "void inc(int x)  { x = x + 1; }   // by value\nvoid inc(int* x) { *x = *x + 1; } // by pointer\nvoid inc(int& x) { x = x + 1; }   // by reference\n\nint a = 5;\ninc(a);    // by value\ninc(&a);   // by pointer\ninc(a);    // by reference", lang: "cpp" },
      choices: [
        { text: "7 — by value leaves `a` at 5, then by pointer makes it 6, then by reference makes it 7", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "`&r` equals `&x` (a reference is no new storage), but `&p` is a different address (the pointer is its own object)", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "A pointer holds an **address**, not an integer; `5` is not an address, so only `p = &x` is valid", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "You do, explicitly with `delete` / `delete[]` — there is no garbage collector", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "It marks the memory free to reuse but leaves `p`'s value unchanged — `p` is now a **dangling** pointer", correct: true },
    ...
    },
    {
      ...
      choices: [
        { text: "Calling `delete` twice on the same memory — undefined behavior; it may crash, corrupt the heap, or appear to work", correct: true },
