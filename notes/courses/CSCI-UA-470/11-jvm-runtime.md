...
A single program passes through seven stages, from source to reclaimed memory.
This note follows the same seven steps, and the demos below animate several of
them.
...
Portability comes from moving the platform-specific part into the JVM. Each platform needs its own JVM implementation, but the same `.class` file can be fed to all of them.

## The JVM at a glance

The lecture diagram separates the JVM into three major parts, the class loaders, the runtime areas, and the execution engine, plus the Java Native Interface (JNI) that bridges out to platform-native libraries (`.dll`, `.so`, `.dylib`):

```artifact src=demos/jvm-architecture-map.jsx static
```

Every section that follows covers one box of this map, and each box corresponds to a stage of the seven-step table above. The two machine models below fill in the runtime areas (stacks, heap, method area). Class loading and verification belong to the class loaders, which perform the load and verify stages. The interpreter, JIT, and garbage collector make up the execution engine, which performs execute, optimize, and reclaim. A `native` call crosses JNI.
...
- **CPU** — includes components such as a control unit and an arithmetic logic unit.
- **Registers** — used directly by native instructions.
- **RAM** — contains areas such as stack, heap, global/static storage, and code.
- **Persistent storage** — an HDD or SSD.
...
Put the two machines side by side on the same sum `a + b`. The register machine
names every operand (`add %rbx, %rax`); the stack machine names none, leaving
them implicit on the operand stack:

```artifact src=demos/register-vs-stack.jsx static
```

Bytecode names no registers because it does not need to know how many a given
CPU has. The JVM maps the operand stack onto real registers when it runs. The
trace below steps through the stack machine on its own, running
`iload`/`iadd`/`ireturn` over one operand stack:
...
`NEW` reads the blueprint from the Method Area, allocates the object on the
Heap, and pushes a reference onto the operand stack. Every later field access
follows that reference. Once the reference goes away, the object becomes
unreachable, and garbage collection takes over from there.

## Inside the JVM: the three subsystems

A class meets the three subsystems of the map above in order: the class loaders bring it in, the runtime areas hold its state, and the execution engine runs its bytecode. JNI is the exit to native code.
...
The three loaders form a parent-first delegation chain. A loader does not load a
requested class immediately. It first asks its parent, which asks its own
...
Before any bytecode runs, the JVM verifies each loaded `.class` during the link
step (the `verify` in load → link → initialize). Verification rejects malformed
or hostile bytecode up front, so the interpreter and JIT can then run it without
re-checking safety on every instruction. It confirms, among other things:
...
A `.class` produced by a different or malicious tool therefore cannot corrupt
the JVM: it must pass the same verification as any other.
...
The execution engine runs bytecode. The lecture lists:
...
The interpreter and the JIT compiler work together. Every method starts interpreted, which gives fast startup, and once it becomes hot the JIT compiles it to native code for the remaining calls:
...
self-referential island with no path from a root is never marked, so it gets
swept.

```artifact src=demos/gc-reachability.jsx static
```

Java execution therefore runs a longer path than "source code runs on the CPU." The full path is: source code → bytecode → JVM loading/runtime/execution machinery → native work on the physical machine.

### Native calls (JNI)

Some methods are declared `native`. Their body lives in a platform library as
compiled native code (`.so` on Linux, `.dll` on Windows, `.dylib` on macOS)
instead of in the `.class` file's bytecode. Calling one crosses the Java Native
Interface (JNI): the thread leaves the ordinary bytecode path, uses its native
method stack instead of the usual JVM stack, runs the native function, and
returns the result back into bytecode execution.

JNI is how Java reaches operating-system and hardware facilities (file I/O,
graphics, vendor libraries) that pure bytecode cannot express. It is also where
portability stops. The native library is platform-specific, so a program that
calls into one is only as portable as that library.
