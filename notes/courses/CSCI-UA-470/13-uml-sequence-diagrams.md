...
A use-case oval such as `Make order` is a single word for a whole conversation. The sequence diagram is where that conversation is written out: which objects take part, what messages they send, and — crucially — **in what order**.

> A sequence diagram illustrates the **objects that participate in a use case** and the **messages that pass between them over time**. Its whole emphasis is **time ordering**.
...
- **Deletion** — an **X** at the bottom of the object's lifeline, where it is destroyed (by another object, or by itself — *self-deletion*).

Because time runs downward, an object created halfway down the page simply has a shorter lifeline that starts lower — the diagram shows its whole lifetime at a glance.

## Combined fragments: conditions and loops

Straight-line message order is the common case, but real interactions branch and repeat. UML wraps a span of messages in a **combined fragment** — a labelled frame with a tab in its top-left corner naming the operator, and a **guard** in `[square brackets]`:
...
The `alt` fragment is the richest — here a bank clears a check, taking one branch when the balance covers the amount and the `[else]` branch when it does not:

```artifact src=demos/sequence-fragments.jsx static
```

An `opt` is just this shape with a single branch and no divider; a `loop` is the same frame whose body runs more than once.

## Worked example: "Make order" at the coffee shop

This is the payoff — a single use case turned into the interaction that realises it. The `Make order` oval from the coffee-shop use-case diagram becomes a conversation between four objects: the **customer** pays the **cashier**, the cashier hands the job to the **barista**, the barista passes the finished drink to the **receptionist**, who checks its quality (a self-call) and returns the order.
...
> **On the return arrows.** This diagram (following the lecture) draws returns to whoever the story follows next — the price back to the customer, the finished order straight back to the customer. Strict UML would instead unwind the call stack one frame at a time, returning each call to its *immediate* caller (receptionist → barista → cashier → customer). The collapsed form trades that rigor for a readable narrative; both are common in practice.

The lecture built this up in versions — first `Customer → Cashier → Barista`, then adding the receptionist and the quality check — which is exactly how the activation bars grow: each new collaborator adds a stack frame further down the page. Earlier versions simply have fewer participants and messages; the shape is the same.
...
> Where this sits in the course: a sequence diagram is the **Interaction**-family companion to the use-case diagram in [note 12](note.html?course=CSCI-UA-470&note=12-uml-use-case-diagrams). A use case names a goal from the outside; the sequence diagram writes out the message-by-message collaboration that fulfils it — the objects, the calls between them, and the order in time. Both stay at the level of design notation, above the C++/Java implementation detail of the earlier notes.
